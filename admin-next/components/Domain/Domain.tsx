...
const Domain: FC = () => {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    return UpdateCustomDomainMutation.commit({input})
      .then(response => {
        ...
        if (errorCode === 'CNAME_NOT_VALID') {
          setError('customDomain', {
            type: 'manual',
            message: isRootDomain() ? intl.formatMessage({id: 'root.domain.not.valid.error'}) : intl.formatMessage({id: 'cname.not.valid.error'})
          })
        }
  ...
  return (
    <Box bg="white" p={6} borderRadius="8px" mb={8} as="form" id={formName} onSubmit={handleSubmit(onSubmit)}>
      <Heading as="h4" color="blue.800" fontWeight={600} mb={4}>{intl.formatMessage({id: 'domain.name'})}</Heading>
      <Flex mb={4} justifyContent="space-between">
        <Box flex={0.5}>
          <Box width="560px">
            <Text fontWeight={600}>{intl.formatMessage({id: 'capco.subdomain'})}</Text>
            <Text>
              {intl.formatMessage({id: 'capco.subdomain.description'})}
            </Text>
          </Box>
        </Box>
        <FormControl name="subDomain" control={control} flex={0.5} isRequired>
          <FormLabel
            htmlFor="subDomain"
            label={intl.formatMessage({id: 'sub-domain'})}
          />
          <FieldInput
            id="subDomain"
            name="subDomain"
      ...
      <Flex>
        <Box flex={0.5}>
          <Box width="560px">
            ...
            <Text>
              {intl.formatMessage({id: 'custom-domain.description'})}
            </Text>
          </Box>
        </Box>
        <Box flex={0.5}>
          <FormControl name="customDomain" control={control} flex={0.5}>
            <FormLabel
              ...
              label={intl.formatMessage({id: 'custom-domain-name'})}
            />
            <FormGuideline>
              {
                isRootDomain() ? intl.formatMessage({id: 'root.domain.config'}, {
                    link: <Button as="a" variant="link" target="_blank" href="https://aide.cap-collectif.com/article/274-cname">A</Button>
                  }) :
                  (intl.formatMessage({id: 'domain.must.point.to.cname'}, {
                    link: <Button as="a" variant="link" target="_blank" href="https://aide.cap-collectif.com/article/274-cname">CNAME</Button>
                  }))
              }
            ...
            <InputGroup>
              {isSubmitting && (
                <Box flex={1} border="1px solid" borderColor="gray.300" borderRadius="normal"
                     lineHeight={CapUILineHeight.Base} px={3} py={1}>
                  <Tag variantColor="yellow" onRemove={() => {
              ...
              {
                status === 'ACTIVE' && (
                  <Box flex={1} border="normal" borderColor="gray.300" borderRadius="normal"
                       lineHeight={CapUILineHeight.Base} px={3} py={1}>
                  ...
                  <FieldInput
                    ...
                    rules={{
                      pattern: {
                        ...
                        value: /^((?!-))((xn--)?[a-z0-9][a-z0-9-_]{0,61}[a-z0-9]{0,1}\.){0,}(xn--)?([a-z0-9\-]{1,61}|[a-z0-9-]{1,30}\.[a-z]{2,})$/i,
                        message: intl.formatMessage({id: 'custom.domain.not.valid.error'})
                      }
                    }}
                  />
                ...
                isSubmitting && (
                  ...
                  >
                    <Tooltip label={intl.formatMessage({id: 'custom-domain-verification-help-message'})}>
                      <Text>
                        {intl.formatMessage({id: 'ongoing-check'})}
                      </Text>
                    </Tooltip>
                  </Button>
                )
              }
              {
                (status === 'IDLE' && !isSubmitting) && (
                  <Button
                    variant="primary"
                    type="submit"
                    disabled={!customDomainFieldValue}
                  >
      ...
      <DeleteCustomDomainModal siteSettings={query.siteSettings} show={isOpen} onClose={onClose}
