...
const IconAndText = ({
  ...
  <Flex alignItems="center" spacing={0} width={width} justifyContent={justifyContent} mr={mr}>
    {icon ? <Icon size={CapUIIconSize.Md} name={icon} color={color || 'neutral-gray.500'} /> : null}
    {children ? (
      <Text
        as="span"
        fontSize="10px"
  ...
  return (
    <Card className={className}>
      <Flex mb={2} justify="start">
        {address ? (
          <IconAndText icon={CapUIIcon.PinO} mr={6} truncateAt={100} color="neutral-gray.900">
      ...
      <Flex justify="space-between" sx={{ '*': { textDecoration: 'none !important' } }} height="100%">
        <Flex direction="column" width="59.7%">
          ...
          <Text as="div" fontSize="11px" color="neutral-gray.900" lineHeight="initial" className="description">
        ...
        <ImageContainer url={url}>
          {media ? (
            <img src={media?.url} alt={media?.name} style={{ objectFit: 'cover', height: '100%', width: '100%' }} />
          ) : (
            <>
              {category?.icon ? (
                <Box position="absolute" left="calc(50% - 2rem)" top="calc(50% - 2rem)">
                  <Icon size={CapUIIconSize.Xxl} name={convertIconToDs(category.icon)} color="white" />
                </Box>
              ) : null}
              <CategoryBackground color={category?.color || '#C4C4C4'} viewBox="5 0 240 80" height="100%" />
            </>
          )}
        </ImageContainer>
      </Flex>
