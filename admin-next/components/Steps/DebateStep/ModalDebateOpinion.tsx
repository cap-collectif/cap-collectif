...
const updateDebateOpinion = async (
  input: FormValues,
  debateOpinionId: string,
  onClose: () => void,
  intl: IntlShape,
) => {
  const author = input.author?.value ?? ''
...
export const ModalDebateOpinion: React.FC<Props> = ({ onClose, type, opinion: opinionRef, debate: debateRef }) => {
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        ...
        <form>
          ...
          <FormControl name="title" control={control}>
            <FormLabel htmlFor="title" label={intl.formatMessage({ id: 'global.review' })} />
            <Controller
              name="body"
              control={control}
              ...
              render={({ field, fieldState }) => {
                ...
                return (
                  <>
                    <Jodit id="body" onChange={onChange} value={value} platformLanguage="fr" />
      ...
      <Modal.Footer>
        <ButtonGroup>
          <Button variantSize="medium" variant="secondary" variantColor="hierarchy" onClick={onClose}>
