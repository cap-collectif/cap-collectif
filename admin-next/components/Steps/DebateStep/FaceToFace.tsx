...
import {Flex, Text, Button, toast} from '@cap-collectif/ui'
...
type OpinionForAndAgainst = Record<ForOrAgainstValue, OpinionFaceToFace | null>
...
const FaceToFace = ({ debate: debateRef }: Props) => {
    ...
    const [opinionSelected, setOpinionSelected] = React.useState<
        false | OpinionFaceToFace | ForOrAgainstValue
    >(false);
            ...
            <Flex direction="column" spacing={6}>
                {opinionForAndAgainst.FOR?.id ? (
                    <DebateOpinion
                        ...
                        onDelete={() =>
                            deleteDebateOpinion(
                                opinionForAndAgainst.FOR?.id as string,
                                debate.id,
                                intl,
                            )
                        }
                ...
                {opinionForAndAgainst.AGAINST?.id ? (
                    <DebateOpinion
                        ...
                        onDelete={() =>
                            deleteDebateOpinion(
                                opinionForAndAgainst.AGAINST?.id as string,
                                debate.id,
                                intl,
                            )
                        }
