...
import {
    CollectStepStatusesList_step$key,
    ProposalStepStatusColor,
} from '@relay/CollectStepStatusesList_step.graphql';
    ...
    if (statuses) {
        return statuses.map(status => {
            if (status.color === bgColor) {
                ...
                const index = Object.values(StatusColorsEnum).indexOf(
                    status.color as StatusColorsEnum,
                );
                return {
                    ...status,
