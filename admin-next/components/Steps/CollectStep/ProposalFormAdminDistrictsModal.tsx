...
const ProposalFormAdminDistrictsModal: React.FC<ProposalFormAdminDistrictsModalProps> = ({
    ...
    const { handleSubmit, formState, resetField, control, watch, setValue } =
        useForm<ProposalFormAdminDistrictsModalFormValues>({
            defaultValues: {
                name: initialValue?.translations?.find(
            ...
            resolver: yupResolver(
                yup.object().shape({
                    ...
                    geojson: yup.string().nullable().test(
                        'geojson',
                        intl.formatMessage({
                            id: 'admin.fields.proposal.map.zone.geojson.invalid',
                        }),
                        function (this: yup.TestContext, fieldValue: any): boolean {
        ...
        <Modal
            ...
            {({ hide }) => (
                <>
                    <Modal.Header>
                        <Modal.Header.Label>
                            {intl.formatMessage({ id: 'proposal-form' })}
                        </Modal.Header.Label>
                        <Heading id="district-modal-title-lg">
                            {intl.formatMessage({
                                id: !isUpdating
                                    ? 'add.geographical.area'
                                    : 'district_modal.update.title',
                            })}
                        </Heading>
                    </Modal.Header>
