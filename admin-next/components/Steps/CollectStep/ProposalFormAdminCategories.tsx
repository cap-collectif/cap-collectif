...
import { ButtonGroup, ButtonQuickAction, CapUIIcon, CapUIIconSize, Flex, Icon } from '@cap-collectif/ui'
...
const ProposalFormAdminCategories: React.FC<ProposalFormAdminCategoriesProps> = ({ query: queryRef, control }) => {
              ...
              <ListCard.Item backgroundColor="white" align="center" py={0} key={category.id}>
                ...
                <ButtonGroup>
                  <ProposalFormAdminCategoriesModal
                    ...
                    update={update}
                    // @ts-ignore
                    colors={proposalCategoryOptions.colors.map(color => color.replace('COLOR_', '#').toLowerCase())}
                    // @ts-ignore
                    icons={proposalCategoryOptions.icons.map(icon => icon.toLowerCase().replace(/_/g, '-'))}
