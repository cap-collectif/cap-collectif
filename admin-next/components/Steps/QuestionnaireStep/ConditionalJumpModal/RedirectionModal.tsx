...
export const RedirectionModal: React.FC<Props> = ({ onClose, onSuccess, isNewJump = false }) => {
    const intl = useIntl();
    ...
    const questionsWithNoJumps = questions.filter(
        (x: QuestionIds) =>
            !questionsWithJumps.some((f: QuestionIds) => f.id && x.id && f.id === x.id),
    );

    if (!isNewJump)
        questionsWithNoJumps.push(questions.find((q: QuestionIds) => q.id === temporaryJump.id));
            ...
            <Modal.Body>
                <Box bg="gray.100" p={6} borderRadius="accordion">
                    <FormControl name={`temporaryJump.id`} control={control}>
                        <FormLabel
                            htmlFor={`temporaryJump.id`}
                            label={intl.formatMessage({ id: 'by-answering-question' })}
                        />
                        <FieldInput
            ...
            <Modal.Footer>
                <ButtonGroup>
                    ...
                    <Button
                        ...
                        disabled={
                            !temporaryJump?.id || !temporaryJump?.alwaysJumpDestinationQuestion?.id
                        }
