...
const ConsultationModel: React.FC<Props> = ({ query: queryRef, consultationFormKey, currentConsultationId, children }) => {
    const intl = useIntl();
    ...
    const consultationsWithStep = owner.consultationsWithStep.edges.map(edge => edge?.node).filter(node => node.id !== currentConsultationId) ?? []
