...
export const WYSIWYGRender = (props: Props) => {
  ...
  if (typeof value === 'string' && isPredefinedTraductionKey(value)) {
    return <div className={`${className || ''} ql-editor wysiwyg-render`}>{translateContent(value)}</div>
  }
  ...
  if (tagName) {
    ...
    return raw ? child : <div className={`${className || ''} ql-editor wysiwyg-render`}>{child}</div>
  }
