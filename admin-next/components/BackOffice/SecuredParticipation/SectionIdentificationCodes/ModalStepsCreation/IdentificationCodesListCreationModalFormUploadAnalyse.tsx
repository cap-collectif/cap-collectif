      ...
      <InfoMessage.Title withIcon>
        ...
        <FormattedHTMLMessage
          ...
          values={{
            ...
            lines:
              data.invalidLines.length > 1 ? data.invalidLines.slice(0, -1).toString() : data.invalidLines.toString(),
...
const IdentificationCodesListCreationModalFormUploadAnalyseNoValid: FC = () => {
  ...
  return (
    <InfoMessage variant="danger" mt={2}>
      ...
      <InfoMessage.Content>{intl.formatMessage({ id: 'check-your-csv' })}</InfoMessage.Content>
    </InfoMessage>
  )
}
    ...
    <>
      {data.validData.length > 0 ? (
        <>
          ...
          {data.invalidLines.length > 0 && <IdentificationCodesListCreationModalFormUploadAnalyseInvalid data={data} />}
          {data.duplicateLines.length > 0 && (
            <IdentificationCodesListCreationModalFormUploadAnalyseDuplicate data={data} />
          )}
        </>
      ) : (
        <>
