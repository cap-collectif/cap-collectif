...
import EmptyMessage from 'components/BackOffice/UI/Table/EmptyMessage'
...
export const PostListQuery = graphql`
  ...
  @refetchable(queryName: "PostListPaginationQuery") {
    posts(first: $count, after: $cursor, query: $term, affiliations: $affiliations, orderBy: $orderBy)
    ...
    >
      <Table.Thead>
        <Table.Tr>
          ...
          {isAdmin || isAdminOrganization ? <Table.Th>{intl.formatMessage({ id: 'global.owner' })}</Table.Th> : null}
          <Table.Th>
            <Text lineHeight="sm">{intl.formatMessage({ id: 'admin.projects.list.author' })}</Text>
          </Table.Th>
          <Table.Th>
            <Text lineHeight="sm">{intl.formatMessage({ id: 'global.participative.project' })}</Text>
          </Table.Th>
          <Table.Th>
            ...
            post =>
              post && (
                <Table.Tr key={post.id} rowId={post.id} data-cy="post-item">
                  <PostItem post={post} connectionName={posts.__id} viewer={viewer} />
                </Table.Tr>
              ),
          )}
      </Table.Tbody>
