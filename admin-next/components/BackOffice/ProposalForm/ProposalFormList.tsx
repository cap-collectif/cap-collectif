...
export const PROPOSALFORM_OWNER_FRAGMENT = graphql`
  ...
  @refetchable(queryName: "ProposalFormListPaginationQuery") {
    proposalForms(first: $count, after: $cursor, query: $term, affiliations: $affiliations, orderBy: $orderBy)
      @connection(key: "ProposalFormList_proposalForms", filters: ["query", "orderBy", "affiliations"]) {
      __id
      totalCount
      edges {
        node {
            ...
            proposalForm =>
              proposalForm && (
                <Table.Tr key={proposalForm.id} rowId={proposalForm.id} data-cy="proposalform-item">
                  <ProposalFormItem viewer={viewer} proposalForm={proposalForm} connectionName={proposalForms.__id} />
                </Table.Tr>
              ),
          )}
      </Table.Tbody>
