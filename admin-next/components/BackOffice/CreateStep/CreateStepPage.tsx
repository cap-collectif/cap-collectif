...
import { getDefaultStepConfig, StepTypeEnum } from '@components/BackOffice/CreateStep/defaultStepConfig'
import { createStep } from '@components/BackOffice/CreateStep/createStep'
...
const CreateStepPage: React.FC<Props> = ({ projectId }) => {
  ...
  const stepsConfig: Array<Step> = defaultStepsConfig.filter(step => {
    if (!hasCollectStep) {
      return ![StepTypeEnum.VOTE, StepTypeEnum.ANALYSIS, StepTypeEnum.RESULT].includes(step.type)
    }
    return true
  })
  ...
  return (
    <Flex>
      <Box bg="white" width="70%" p={6} height="100%" borderRadius="8px">
        ...
        <Button as="a" variant="secondary" variantColor="hierarchy" variantSize="big" href={project?.adminAlphaUrl}>
      ...
      >
        <HelpMessage variant="info">{stepsConfig.find(step => step.type === stepHovered)?.helpText}</HelpMessage>
      </Box>
    </Flex>
  )
}
