...
import ConsultationItem from '@components/BackOffice/ConsultationList/ConsultationItem'
...
export const CONSULTATION_CONSULTATION_OWNER_FRAGMENT = graphql`
  ...
  @refetchable(queryName: "ConsultationListPaginationQuery") {
    ... on ConsultationOwner {
      consultations(first: $count, after: $cursor, query: $term, orderBy: $orderBy)
        @connection(key: "ConsultationList_consultations", filters: ["query", "orderBy"]) {
        __id
        totalCount
        edges {
          node {
      ...
      >
        {consultations?.edges
          ...
          .map(
            consultation =>
              consultation && (
                <Table.Tr key={consultation.id} rowId={consultation.id} data-cy="consultation-item">
                  <ConsultationItem consultation={consultation} connectionName={consultations.__id} />
                </Table.Tr>
              ),
          )}
      </Table.Tbody>
