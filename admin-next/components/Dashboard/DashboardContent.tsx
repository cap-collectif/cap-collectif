...
const DashboardContent: FC<DashboardContentProps> = ({ viewer: viewerFragment }) => {
  ...
  const [dateRange, setDateRange] = useUrlState('dateRange', JSON.stringify(DEFAULT_FILTERS.dateRange))
    ...
    <DashboardContext.Provider value={contextValue}>
      <Flex direction="column" spacing={8}>
        ...
        <Flex direction="row" justify="flex-start" overflow="auto" spacing={8}>
          <Suspense fallback={<SectionPlaceholder name="visitors" setQueryOptions={setQueryOptionsSections} />}>
            <SectionVisitors queryOptions={queryOptionsSections['visitors']} />
          </Suspense>

          {projectId === 'ALL' && (
            <Suspense fallback={<SectionPlaceholder name="registrations" setQueryOptions={setQueryOptionsSections} />}>
              <SectionRegistrations queryOptions={queryOptionsSections['registrations']} />
            </Suspense>
          )}

          <Suspense fallback={<SectionPlaceholder name="contributors" setQueryOptions={setQueryOptionsSections} />}>
            <SectionContributors queryOptions={queryOptionsSections['contributors']} />
          </Suspense>

          <Suspense fallback={<SectionPlaceholder name="pageViews" setQueryOptions={setQueryOptionsSections} />}>
            <SectionPageViews queryOptions={queryOptionsSections['pageViews']} />
          </Suspense>
        </Flex>

        <Flex direction="row" justify="space-between" spacing={8}>
          <Suspense fallback={<SectionPlaceholder name="participations" setQueryOptions={setQueryOptionsSections} />}>
            <SectionParticipations queryOptions={queryOptionsSections['participations']} />
          </Suspense>

          <Flex direction="column" spacing={8} width="50%">
            <Suspense
              fallback={<SectionPlaceholder name="mostVisitedPages" setQueryOptions={setQueryOptionsSections} />}
            >
              <SectionMostVisitedPages queryOptions={queryOptionsSections['mostVisitedPages']} />
            </Suspense>

            <Suspense
              fallback={<SectionPlaceholder name="topContributors" setQueryOptions={setQueryOptionsSections} />}
            >
              <SectionTopContributors queryOptions={queryOptionsSections['topContributors']} />
            </Suspense>
          </Flex>
        </Flex>

        <Flex direction="row" justify="space-between" spacing={8}>
          <Suspense fallback={<SectionPlaceholder name="traffic" setQueryOptions={setQueryOptionsSections} />}>
            <SectionTraffic queryOptions={queryOptionsSections['traffic']} />
          </Suspense>

          <Suspense
            fallback={<SectionPlaceholder name="proposalCategories" setQueryOptions={setQueryOptionsSections} />}
          >
            <SectionProposalCategories queryOptions={queryOptionsSections['proposalCategories']} />
          </Suspense>
        </Flex>
      </Flex>
