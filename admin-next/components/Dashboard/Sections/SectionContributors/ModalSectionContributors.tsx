...
const ModalSectionContributors: FC<ModalSectionContributorsProps> = ({
  ...
  const anonymousContributors = useFragment(ANONYMOUS_CONTRIBUTORS_FRAGMENT, anonymousContributorsFragment)
  const intl = useIntl()
  ...
  return (
    <Modal
      ...
      ariaLabel={intl.formatMessage({ id: 'global.contributor.dynamic' }, { num: totalContributorsCount })}
      size={CapUIModalSize.Lg}
    >
      <Modal.Header>
        <Heading as="h4" color="blue.900">
          {totalContributorsCount}{' '}
          {intl.formatMessage({ id: 'global.contributor.dynamic' }, { num: totalContributorsCount })}
        </Heading>
      </Modal.Header>
      <Modal.Body spacing={5}>
        ...
        <InfoMessage variant="info">
          ...
          <InfoMessage.Content>{intl.formatMessage({ id: 'definition-participant-word' })}</InfoMessage.Content>
        </InfoMessage>

        <LineChart
          data={formatValues(totalContributorsValues, intl)}
          label={intl.formatMessage({ id: 'global.contributor.dynamic' }, { num: totalContributorsCount })}
          height="270px"
          withAxis
          withGrid
        />
