    ...
    <>
      <Box as="button" type="button" onClick={onOpen} flex={1} maxWidth="33%">
        <SmallChart
          ...
          label={intl.formatMessage({ id: 'global.registration.dynamic' }, { num: registrations?.totalCount || 0 })}
          data={registrations ? formatValues(registrations.values, intl) : []}
        />
      </Box>

      {registrations && <ModalSectionRegistrations show={isOpen} onClose={onClose} registrations={registrations} />}
    </>
  )
}
