...
const SectionMostVisitedPages: FC<SectionMostVisitedPagesProps> = ({ queryOptions }) => {
  ...
  const data = useLazyLoadQuery<SectionMostVisitedPagesQueryType>(QUERY, getVariablesQuery(filters), queryOptions)
  const intl = useIntl()
  ...
  return (
    <>
        ...
        <Section spacing={6} border="normal" borderColor="gray.150">
          ...
          {mostVisitedPages.values.slice(0, 3).map((value, idx) => (
            <ViewChart
              ...
              label={value.key === '/' ? intl.formatMessage({ id: 'navbar.homepage' }) : `/${formatLabel(value.key)}`}
              url={value.key}
            />
          ))}
        </Section>
      ...
      <ModalSectionMostVisitedPages show={isOpen} onClose={onClose} mostVisitedPages={mostVisitedPages} />
    </>
  )
}
