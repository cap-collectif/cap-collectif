...
const SectionProposalCategories: FC<SectionProposalCategoriesProps> = ({ queryOptions }) => {
  ...
  const data = useLazyLoadQuery<SectionProposalCategoriesQueryType>(QUERY, getVariablesQuery(filters), queryOptions)
  const intl = useIntl()

  const { mostUsedProposalCategories } = data.analytics

  if (!mostUsedProposalCategories || mostUsedProposalCategories?.totalCount === 0)
    return <SectionProposalCategoriesEmpty />
  ...
  return (
    <Section width="50%" spacing={6} border="normal" borderColor="gray.150">
      ...
      <PieChart
        percentages={formatCategories(mostUsedProposalCategories.values, mostUsedProposalCategories.totalCount)}
      />
    </Section>
  )
}
