...
const ModalCasConfiguration: FC<ModalCasConfigurationProps> = ({
  ...
  const defaultValues =
    ssoConfiguration && isEditing
      ? {
          ...
          casVersion: casVersionOptions.find(option => option.value === ssoConfiguration.casVersion)?.value,
  ...
  return (
    ...
    >
      <Modal.Header>
        <Modal.Header.Label>{intl.formatMessage({ id: 'add-cas-authentication-method' })}</Modal.Header.Label>
        ...
        <Button
          ...
          onClick={e => {
            methods.handleSubmit((data: FormValues) =>
              onSubmit(data, intl, ssoConnectionName, isEditing || false, ssoConfiguration?.id || undefined),
            )(e)
            onClose()
          }}
        >
