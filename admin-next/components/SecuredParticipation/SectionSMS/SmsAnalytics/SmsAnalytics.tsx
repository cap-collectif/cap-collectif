...
const getAlertMessage = (isSuperAdmin: boolean, status: RemainingSmsCreditStatus, intl: IntlShape, creditCount: number):string => {
    if(status === "VERY_LOW") {
        ...
        <Flex direction="row" spacing={4}>
            <CountSection variant={STATUS_COUNT[remainingCredits.status]}>
                <Flex direction="row" align="center" spacing={2}>
                    ...
                    {hasAlertMessage && (
                        <Tooltip label={getAlertMessage(viewerSession.isSuperAdmin, remainingCredits.status, intl, remainingCredits.amount)}>
                            <Icon
