...
export const getSideBarItemsFiltered = (isAdmin: boolean, allFeatureFlags: FeatureFlags): typeof sideBarItems => {
    return sideBarItems.reduce<typeof sideBarItems>((acc, sideBarItem) => {
        ...
        const hasItemFeatureRequired = (sideBarItem.featuresRequired as FeatureFlagType[]).every(featureRequired => allFeatureFlags[featureRequired])
        ...
        if(isItemForAdminOnly || isItemForAll && hasItemFeatureRequired) {
            ...
            sideBarItem.items = sideBarItem.items.filter(subItem => {
                ...
                const hasSubItemsFeatureRequired = (subItem.featuresRequired as FeatureFlagType[]).every(featureRequired => allFeatureFlags[featureRequired])

                if (isSubItemForAdminOnly || isSubItemForAll && hasSubItemsFeatureRequired) return subItem;
            });
