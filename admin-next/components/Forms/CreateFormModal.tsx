...
const CreateFormModal: React.FC<CreateFormModalProps> = ({
  ...
  const consultationsCount = viewer?.consultations?.totalCount
    ...
    switch (values.type) {
      ...
      case 'QUESTIONNAIRE_ANALYSIS': {
        const hasQuestionnaire =
          values.type === 'QUESTIONNAIRE' ? questionnairesCount > 0 : analysisQuestionnairesCount > 0
        const types =
          values.type === 'QUESTIONNAIRE'
            ? (['QUESTIONNAIRE', 'VOTING'] as Array<QuestionnaireType>)
            : (['QUESTIONNAIRE_ANALYSIS'] as Array<QuestionnaireType>)
        const response = await CreateQuestionnaireMutation.commit(
          {
            ...
            connections: [
              ConnectionHandler.getConnectionID(owner?.id, 'QuestionnaireList_questionnaires', {
                query: term || '',
      ...
      case 'CONSULTATION': {
        const hasConsultation = consultationsCount > 0
        const response = await CreateConsultationMutation.commit(
          {
            input: {
              ...input,
            },
            connections: [
              ConnectionHandler.getConnectionID(owner.id, 'ConsultationList_consultations', {
                query: term || '',
                orderBy: { field: 'CREATED_AT', direction: orderBy },
              }),
            ],
        ...
        if (!response.createConsultation?.consultation) {
          return mutationErrorToast(intl)
        }
            ...
            <form id={formName}>
              <FormControl name="title" control={control} isRequired>
                ...
                <FieldInput
                  ...
                  id="title"
                  name="title"
                  placeholder={intl.formatMessage({ id: 'create-form-placeholder' })}
                  control={control}
                  minLength={2}
                  maxLength={255}
                />
              ...
              <FormControl name="type" control={control} isRequired>
                <FormLabel htmlFor="type" label={intl.formatMessage({ id: 'global.type' })} />
                <FieldInput
                  name="type"
                  control={control}
