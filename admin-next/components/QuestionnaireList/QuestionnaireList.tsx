...
export const QUESTIONNAIRE_OWNER_FRAGMENT = graphql`
  ...
  @refetchable(queryName: "QuestionnaireListPaginationQuery") {
    questionnaires(
      ...
      orderBy: $orderBy
      types: $types
    ) @connection(key: "QuestionnaireList_questionnaires", filters: ["query", "orderBy", "affiliations", "types"]) {
      __id
      totalCount
      edges {
        node {
...
const QuestionnaireList: React.FC<QuestionnaireListProps> = ({
  ...
  const { data, loadNext, hasNext, refetch } = usePaginationFragment(QUESTIONNAIRE_OWNER_FRAGMENT, questionnaireOwner)
