...
`;
const OrganizationConfigFormDeleteMemberModal: React.FC<
    OrganizationConfigFormDeleteMemberModalProps
> = ({ userName, status, userId, organization: orgRef, inviteId }) => {
    const intl = useIntl();
    const organization = useFragment(FRAGMENT, orgRef);
    const [isSubmitting, setIsSubmitting] = React.useState(false);
    const onSubmit = () => {
        const membersConnectionId = formatConnectionPath(
        ...
        <Modal
            ...
            {({ hide }) => (
                <>
                    ...
                    <Modal.Body>
                        {intl.formatMessage(
                            { id: 'organization.member.delete.modal.body' },
                            { userName: userName },
                        )}
                    </Modal.Body>
                    <Modal.Footer>
