...
export const getFeatureItemsFiltered = (term: string, intl: IntlShape, allFeatureFlags: FeatureFlags, viewerSuperAdmin: boolean): FeatureItem[] => {
        ...
        .map(featureItem => {
            const groups = featureItem.groups
                .map(group => {
                    ...
                    const items = itemsAsArray.reduce<Items>(
                        (acc, [name , data]) => {
                            const containTerm = term && intl
                                .formatMessage({ id: data.title })
                                .toLowerCase()
                                .includes(term.toLowerCase())

                            const hasDependenciesEnabled = data.dependOf ? data.dependOf.every(featureFlagDependency => allFeatureFlags[featureFlagDependency]) : true;
                ...
                .filter(group => {
                    ...
                    if(Object.keys(group.items).length > 0 && hasRoleRequired) return group;
                });
