...
const PieChart: FC<PieChartProps> = ({ percentages, ...props }) => {
  ...
  const percentageColors: string[] = [colors.blue['800'], colors.blue['500'], colors.green['500'], colors.orange['500']]
  ...
  const percentagesFormatted = useMemo(
    () =>
      [...displayPercentages, otherPercentages].filter(p => p.value).sort((p1, p2) => (p1.value > p2.value ? -1 : 1)),
      ...
      <Flex direction="column" spacing={3} flex={1}>
        {percentagesFormatted.map(percentage => (
          <Label
            ...
            title={percentage.id === 'other' ? intl.formatMessage({ id: percentage.label }) : percentage.label}
          >
            {`${
              percentage.id === 'other'
