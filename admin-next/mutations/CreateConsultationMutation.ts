...
import type {
    ...
    CreateConsultationMutationResponse,
    CreateConsultationMutationVariables,
} from '@relay/CreateConsultationMutation.graphql';


const mutation = graphql`
    mutation CreateConsultationMutation($input: CreateConsultationInput!, $connections: [ID!]!)
    @raw_response_type {
...
const commit = (
...
): Promise<CreateConsultationMutationResponse> =>
    commitMutation<CreateConsultationMutation>(environment, {
        ...
        updater: (store: RecordSourceSelectorProxy) => {
            ...
            const consultationsTotalCount = parseInt(
                String(consultations.getValue('totalCount')),
                10,
            );
