...
type DebateAnonymousVotesCookie = Record<string, DebateAnonymousVoteValue>
type DebateAnonymousArgumentValue = {
  type: ForOrAgainstValue
  token: string
  id: string
}
type DebateAnonymousArgumentsCookie = Record<string, DebateAnonymousArgumentValue>
type AnonymousReplyArgument = {
  replyId: string
  token: string
}
type ReplyAnonymousArgumentsCookie = Record<string, AnonymousReplyArgument[]>
...
class CookieMonster {
  ...
  hasDebateAnonymousVoteCookie = (debateId: string): boolean => !!this.getDebateAnonymousVoteCookie(debateId)
  addDebateAnonymousVoteCookie = (debateId: string, { type, token }: DebateAnonymousVoteValue): void => {
    const votes: DebateAnonymousVotesCookie = Cookies.get(DEBATE_ANONYMOUS_VOTES_NAME)
      ? JSON.parse(atob(Cookies.get(DEBATE_ANONYMOUS_VOTES_NAME)))
      : {}
    votes[debateId] = {
  ...
  hasDebateAnonymousArgumentCookie = (debateId: string): boolean => !!this.getDebateAnonymousArgumentCookie(debateId)
  addDebateAnonymousArgumentCookie = (debateId: string, { type, token, id }: DebateAnonymousArgumentValue): void => {
    const args: DebateAnonymousArgumentsCookie = Cookies.get(DEBATE_ANONYMOUS_ARGUMENTS_NAME)
      ? JSON.parse(atob(Cookies.get(DEBATE_ANONYMOUS_ARGUMENTS_NAME)))
      : {}
    args[debateId] = {
  ...
  getAnonymousRepliesIds = (questionnaireId: string): string[] => {
    const args: ReplyAnonymousArgumentsCookie = Cookies.get(REPLY_ANONYMOUS_NAME)
      ? JSON.parse(atob(Cookies.get(REPLY_ANONYMOUS_NAME)))
      : {}
  ...
  getHashedAnonymousReplyCookie = (questionnaireId: string, replyId: string): string | null => {
    ...
    if (args[questionnaireId]) {
      const arg: AnonymousReplyArgument | null | undefined = args[questionnaireId].find(
  ...
  removeAnonymousReplyCookie = (questionnaireId: string, replyId: string): void => {
    const args: ReplyAnonymousArgumentsCookie = Cookies.get(REPLY_ANONYMOUS_NAME)
      ? JSON.parse(atob(Cookies.get(REPLY_ANONYMOUS_NAME)))
      : {}
  ...
  processCookieConsent = () => {
    ...
    if (this.isDoNotTrackActive()) {
      if (typeof analyticConsent === 'undefined') {
        Cookies.set('analyticConsentValue', false, {
          expires: 395,
        })
      }
      ...
      if (consentCookie) {
        Cookies.set('hasFullConsent', false, {
          expires: 395,
        })
      }
    }
  ...
  considerFullConsent = () => {
    Cookies.set('hasFullConsent', true, {
      expires: 395,
    })
  ...
  expireCookie = (coockieName: string, domain: string | null | undefined, expireAt: string): void => {
    document.cookie = coockieName + '=; expires=' + expireAt + (domain ? '; domain=.' + domain : '') + '; path=/'
  }
  getCookies = () => {
    let k = 0
  ...
  executeAnalyticScript(changeExpiration: boolean | null | undefined) {
  ...
  executeAdsScript(changeExpiration: boolean | null | undefined) {
