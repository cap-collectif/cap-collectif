...
const isTest = getBaseUrl() === 'https://capco.test' /* || getBaseUrl() === 'https://capco.dev' */
...
const CaptchetatCaptcha = React.forwardRef<HTMLInputElement, Props>(({ onChange, style, disabled = false }, ref) => {
  ...
  // @ts-ignore CAPTCHETAT_BACKEND_URL not on DOM by default
  const backendUrl: string | undefined = canUseDOM ? window.CAPTCHETAT_BACKEND_URL : undefined
  ...
  const fetchImage = React.useCallback(async () => {
    ...
    try {
      setError(null)
      const res = await fetch(`${backendUrl}?get=image&c=${CAPTCHETAT_STYLE}`)
      if (!res.ok) {
        const body = await res.text().catch(() => '(unreadable)')
        console.error(`[CAPTCHETAT] Failed to fetch captcha image: HTTP ${res.status} from ${res.url}`, body)
        setError('registration.constraints.captcha.invalid')
        return
      }
      const json: CaptchetatData = await res.json()
      setData(json)
      setCode('')
      setValidationStatus('idle')
      onChange('')
    } catch (e) {
      console.error(`[CAPTCHETAT] Failed to fetch captcha image from ${backendUrl}:`, e)
      setError('registration.constraints.captcha.invalid')
    }
  }, [backendUrl, onChange])

  const playSound = React.useCallback(async () => {
    if (!backendUrl || !data) return
    try {
  ...
  React.useEffect(() => {
    // In test mode, bypass the widget — CaptchaChecker always returns true
    if (isTest) {
  ...
  if (!backendUrl) {
    return (
      <Box color="red.500" lineHeight="normal" fontSize={CapUIFontSize.BodyRegular} role="alert">
