...
const applyTranslations = (text, locale) => {
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(`${translationsPath}messages+intl-icu.${locale}.xlf`, 'utf8', async (err, data) => {
      const jsonObj = parser.parse(data).xliff.file.body['trans-unit'].sort((a, b) => b.source.length - a.source.length)
...
const readFile = file => {
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(`${snapshotsPath}${file}`, 'utf8', async (err, data) => {
      ...
      const translatedData = await applyTranslations(data, file.endsWith('English.html') ? 'en-GB' : 'fr-FR')
