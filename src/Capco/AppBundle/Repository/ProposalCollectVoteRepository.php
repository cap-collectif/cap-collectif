...
class ProposalCollectVoteRepository extends EntityRepository
{
     ...
     */
    public function exportVotesByProposalAndStep(CollectStep $step, Proposal $proposal): iterable
    {
        $sql = <<<'SQL'
                        select v.created_at, v.publishedAt, v.published, v.is_accounted, v.private, v.id vote_id, v.position
                           ...
                           user_type_translation.name user_type_name
                            ...
                            where v.is_accounted = 1 and v.published = 1 and p.id = :proposal and v.collect_step_id = :stepId
            SQL;
    ...
    /**
     * Get aggregated statistics (vote counts and proposal counts) per user for a CollectStep.
     *
     * @return array{votes: array<string, int>, proposals: array<string, int>}
     */
    public function getUserStatsForStep(CollectStep $step): array
    {
        $connection = $this->_em->getConnection();
        ...
        if ($step->isVotable()) {
            $votesSql = <<<'SQL'
                    ...
                    FROM votes v
                    WHERE v.collect_step_id = :step_id
        ...
        // Count proposals per user for this step (join through proposal_form)
        $proposalsSql = <<<'SQL'
                SELECT
