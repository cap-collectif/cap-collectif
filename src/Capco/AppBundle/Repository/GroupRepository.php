...
class GroupRepository extends EntityRepository
{
    ...
    public function getMembers(Group $group, string $term, ?int $offset = 0, ?int $limit = 10000): array
    {
        ...
        $sql = <<<'SQL'
                        SELECT *
                        FROM (
                            SELECT TO_BASE64(CONCAT('User:', u1.id)) AS userId, u1.email AS email, u1.username AS username, CONCAT(u1.firstname, ' ', u1.lastname) AS fullname, 'MEMBER' AS type
                            FROM user_group g
                            LEFT JOIN user_in_group uig ON uig.group_id = g.id
                            LEFT JOIN fos_user u1 ON u1.id = uig.user_id
                            WHERE g.id = :groupId and (u1.email like :term OR u1.username like :term OR CONCAT(u1.firstname, ' ', u1.lastname) like :term)
                            UNION
                            SELECT null as id, user_invite.email, '' AS username, '' AS fullname, 'INVITATION' AS type
