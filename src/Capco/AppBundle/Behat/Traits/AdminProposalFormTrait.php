...
trait AdminProposalFormTrait
{
     ...
     * @param mixed $tab
     */
    public function iSaveTheProposalForm($tab)
    {
     ...
     * @param mixed $tab
     */
    public function iGoToTheAdminProposalFormTab($tab)
    {
        $page = $this->getCurrentPage();
        $this->iWait(3); // Wait alert to disappear
        $this->getSession()->wait(3000, "$('" . $page->getSelector('proposal form ' . $tab . ' tab') . "').length > 0");
        $page->clickOnTab($tab);
        $this->iWait(1);
    }
}
