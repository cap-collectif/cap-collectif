...
class RecalculateCountersCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Opinion o set o.versionsCount = (
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Opinion o set o.argumentsCount = (
          select count(DISTINCT a.id)
          from CapcoAppBundle:Argument a
          WHERE a.isEnabled = 1 AND a.isTrashed = 0 AND a.expired = 0 AND a.opinion = o
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:Opinion o set o.sourcesCount = (
        ...
        $this->executeQuery('UPDATE opinion AS o
          JOIN
          ...
          ON g.id = o.id
          SET o.connections_count = g.cnt', true
        );

        $this->executeQuery('UPDATE CapcoAppBundle:Opinion a set a.votesCountOk = (
          select count(DISTINCT ov.id)
          from CapcoAppBundle:OpinionVote ov
          where ov.opinion = a AND ov.expired = 0 AND ov.value = 1 group by ov.opinion
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:Opinion a set a.votesCountMitige = (
          select count(DISTINCT ov.id)
          from CapcoAppBundle:OpinionVote ov
          where ov.opinion = a AND ov.expired = 0 AND ov.value = 0 group by ov.opinion
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:Opinion a set a.votesCountNok = (
          select count(DISTINCT ov.id)
          from CapcoAppBundle:OpinionVote ov
          where ov.opinion = a AND ov.expired = 0 AND ov.value = -1 group by ov.opinion
        )');
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:OpinionVersion ov set ov.argumentsCount = (
          select count(DISTINCT a.id)
          from CapcoAppBundle:Argument a
          WHERE a.isEnabled = 1 AND a.isTrashed = 0 AND a.expired = 0 AND a.opinionVersion = ov
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:OpinionVersion ov set ov.sourcesCount = (
          select count(DISTINCT s.id)
          from CapcoAppBundle:Source s
          WHERE s.isEnabled = 1 AND s.isTrashed = 0 AND s.expired = 0 AND s.opinionVersion = ov
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:OpinionVersion ov set ov.votesCountOk = (
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:OpinionVersion ov set ov.votesCountMitige = (
          select count(DISTINCT ovv.id)
          from CapcoAppBundle:OpinionVersionVote ovv
          where ovv.opinionVersion = ov AND ovv.expired = 0 AND ovv.value = 0 group by ovv.opinionVersion
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:OpinionVersion ov set ov.votesCountNok = (
          select count(DISTINCT ovv.id)
          from CapcoAppBundle:OpinionVersionVote ovv
          where ovv.opinionVersion = ov AND ovv.expired = 0 AND ovv.value = -1 group by ovv.opinionVersion
        )');
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Post p set p.commentsCount = (
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Event e set e.commentsCount = (
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Proposal p set p.commentsCount = (
          select count(DISTINCT pc.id)
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.opinionCount = (
          select count(DISTINCT o.id)
          from CapcoAppBundle:Opinion o
          where o.step = cs AND o.isEnabled = 1 AND o.isTrashed = 0 AND o.expired = 0 group by o.step
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.opinionVersionsCount = (
          select count(DISTINCT ov.id)
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.trashedOpinionCount = (
          select count(DISTINCT o.id)
          from CapcoAppBundle:Opinion o
          where o.step = cs AND o.isEnabled = 1 AND o.isTrashed = 1 AND o.expired = 0 group by o.step
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.trashedOpinionVersionsCount = (
          select count(DISTINCT ov.id)
          from CapcoAppBundle:OpinionVersion ov INNER JOIN CapcoAppBundle:Opinion o WITH ov.parent = o
          where o.step = cs AND ov.enabled = 1 AND ov.isTrashed = 1 AND o.isEnabled = 1 AND o.expired = 0 group by o.step
        )');

        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.argumentCount = (
          select count(DISTINCT a.id)
          from CapcoAppBundle:Argument a
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.trashedArgumentCount = (
          select count(DISTINCT a.id)
          from CapcoAppBundle:Argument a
          LEFT JOIN CapcoAppBundle:OpinionVersion ov WITH a.opinionVersion = ov
          LEFT JOIN CapcoAppBundle:Opinion o WITH a.opinion = o
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.sourcesCount = (
          select count(DISTINCT s.id)
          from CapcoAppBundle:Source s
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs set cs.trashedSourceCount = (
          select count(DISTINCT s.id)
          from CapcoAppBundle:Source s
          LEFT JOIN CapcoAppBundle:OpinionVersion ov WITH s.opinionVersion = ov
          LEFT JOIN CapcoAppBundle:Opinion o WITH s.Opinion = o
        ...
        foreach ($consultationSteps as $cs) {
            if ($cs->isOpen() || $this->force) {
                ...
                $this->executeQuery('UPDATE CapcoAppBundle:Steps\ConsultationStep cs
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\CollectStep cs set cs.proposalsCount = (
        ...
        foreach ($collectSteps as $cs) {
            if ($cs->isOpen() || $this->force) {
                $participants = $contributionResolver->countStepContributors($cs);
                $this->executeQuery('UPDATE CapcoAppBundle:Steps\CollectStep cs
                    set cs.contributorsCount = ' . $participants . '
                    where cs.id = \'' . $cs->getId() . '\''
                );
            }
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\QuestionnaireStep qs set qs.repliesCount = (
        ...
        $questionnaireSteps = $this->entityManager->getRepository('CapcoAppBundle:Steps\QuestionnaireStep')->findAll();
        foreach ($questionnaireSteps as $qs) {
            if ($qs->isOpen() || $this->force) {
                $participants = $contributionResolver->countStepContributors($qs);
                $this->executeQuery('UPDATE CapcoAppBundle:Steps\QuestionnaireStep qs
        ...
        $this->executeQuery('UPDATE CapcoAppBundle:Steps\SelectionStep ss set ss.votesCount = (
        ...
        foreach ($selectionSteps as $ss) {
            if ($ss->isOpen() || $this->force) {
                $anonymousParticipants = $container->get('capco.user.repository')->countSelectionStepProposalAnonymousVoters($ss);
                $participants = $contributionResolver->countStepContributors($ss) + $anonymousParticipants;
                $this->executeQuery('UPDATE CapcoAppBundle:Steps\SelectionStep ss
