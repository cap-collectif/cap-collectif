...
describe('Preview|Project.proposals connection', () => {
  it('fetches the proposals of a public project as anonymous', async () => {
    const response = await graphql(ProjectProposalsQuery, { id: global.toGlobalId('Project', 'project6'), isAuthenticated: false }, 'internal');
  ...
  it('fetches the proposals of a private project', async () => {
    const response = await graphql(ProjectProposalsQuery, { id: global.toGlobalId('Project', 'projectIdf'), isAuthenticated: false }, 'internal');
    expect(response.project.proposals).toMatchSnapshot();
    expect(response.viewer).toBe(undefined);
  });

  it('fetches the proposals of a private project as an admin', async () => {
    const response = await graphql(ProjectProposalsQuery, { id: global.toGlobalId('Project', 'projectIdf'), isAuthenticated: true }, 'internal_admin');
    expect(response.project.proposals).toMatchSnapshot();
