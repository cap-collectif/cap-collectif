...
Scenario: Email should be sent to if a message is sent to the comment_create queue
  ...
  """
  And I run "swarrot:consume:comment_create comment_create --env=test --max-execution-time=3"
...
Scenario: Email should be sent to if a message is sent to the comment_update queue
  ...
  """
  And I run "swarrot:consume:comment_update comment_update --env=test --max-execution-time=3"
...
Scenario: Email should be sent to if a message is sent to the comment_delete queue
  ...
  """
  And I run "swarrot:consume:comment_delete comment_delete --env=test --max-execution-time=3"
