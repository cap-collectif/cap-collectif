...
Scenario: Moderator should receive a new argument email if active moderation is enabled
  ...
  And I should see 'notification-content-new-proposal {"{title}":"Nouveau titre","{body}":"Mes modifications blablabla","{createdDate}":"25\/01\/2018","{createdTime}":"19:06:24","{authorName}":"user","{authorLink}":"http:\/\/capco.dev\/profile\/user","{argumentLink}":"http:\/\/capco.dev\/consultations\/projet-de-loi-renseignement\/consultation\/elaboration-de-la-loi\/arguments\/les-enjeux\/nouveau-titre"}' in mail
