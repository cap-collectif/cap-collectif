...
## List Comments
  ...
  Scenario: API client wants to list comments of a proposal
    When I send a GET request to "/api/proposals/1/comments"
  ...
  Scenario: API client wants to find the first comment of a proposal
    When I send a GET request to "/api/proposals/1/comments?limit=1"
    Then the JSON response should match:
    """
    {
      "comments_and_answers_count": @integer@,
  ...
  Scenario: API client wants to find popular comments of a proposal
    When I send a GET request to "/api/proposals/1/comments?filter=popular"
    Then the JSON response should match:
    """
    {
...
## Create Comments
  ...
  Scenario: Anonymous API client wants to add a comment
    When I send a POST request to "/api/proposals/1/comments" with json:
    """
    {
      "authorName": "Kéké",
