...
class Version20151125103922 extends AbstractMigration implements ContainerAwareInterface
{
    ...
    public function up(Schema $schema)
    {
        ...
        foreach ($opinions as $opinion) {
            ...
            if (count($ot) < 1) {
                echo "Error, opinion type for opinion ".$opinion['o_id']." can not be resolved. Please check it.\n";
            } else {
                echo "Updating opinion ".$opinion['o_id']." with opinion type ".$ot[0]['id'].".\n" ;
                $this->connection->update('opinion', ['opinion_type_id' => $ot[0]['id']], ['id' => $opinion['o_id']]);
            }
        }
        ...
        foreach ($opinionTypesSlugs as $ots) {
            if ($ots['count'] > 1) {
                ...
                echo "Error, multiple (".$ots['count'].") opinion types with slug \"".$ots['slug']."\". Attempting to fix it.\n";
                $em = $this
                    ->container
