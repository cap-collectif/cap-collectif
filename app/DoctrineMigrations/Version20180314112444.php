 ...
 */
class Version20180314112444 extends AbstractMigration
{
    /**
     * @param Schema $schema
     */
    ...
    public function postUp(Schema $schema)
    {
        ...
        foreach ($followers as $follower) {
            $this->connection->update('user_following_proposal', ['notified_of' => FollowerNotifiedOfInterface::DEFAULT], ['id' => $follower['id']]);
        }

        $proposals = $this->connection->fetchAll('SELECT id, created_at, author_id from proposal');
        foreach ($proposals as $proposal) {
            $this->connection->update('user_following_proposal', ['notified_of' => FollowerNotifiedOfInterface::ALL], ['user_id' => $proposal['author_id'], 'proposal_id' => $proposal['id']]);
        }
    }
}
