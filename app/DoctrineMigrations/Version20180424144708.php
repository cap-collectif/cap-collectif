...
class Version20180424144708 extends AbstractMigration implements ContainerAwareInterface
{
    ...
    public function importIdeas(EntityManager $em, ProposalForm $proposalForm, array $ideas)
    {
        ...
        foreach ($ideas as $idea) {
            ...
            if ($idea['media_id']) {
                $proposal->setMedia($em->getRepository(Media::class)->findOneBy(['id' => $idea['media_id']]));
            }
            /* -----------------**************************************************---------------- */
            /* -----------------* import ideas votes & create user if no account *---------------- */
            /* -----------------**************************************************---------------- */
            $ideaVotes = $this->connection->fetchAll("SELECT private,created_at,voter_id,username,email,ip_address FROM votes WHERE idea_id = :id", ['id' => $idea['id']]);
