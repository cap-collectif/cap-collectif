...
class Version20150910154228 extends AbstractMigration
{
    ...
    public function up(Schema $schema)
    {
        ...
        $this->addSql('ALTER TABLE opinion_type ADD parent_id INT DEFAULT NULL, ADD consultation_type_id INT DEFAULT NULL, ADD lft INT NOT NULL, ADD rgt INT NOT NULL, ADD root INT DEFAULT NULL, ADD lvl INT NOT NULL');
        $this->addSql('ALTER TABLE opinion_type ADD CONSTRAINT FK_F11F2BF0727ACA70 FOREIGN KEY (parent_id) REFERENCES opinion_type (id) ON DELETE CASCADE');
    ...
    public function postUp(Schema $schema)
    {
        ...
        foreach ($this->steps as $sid => $step) {
            ...
            $this->connection->update('step', ['consultation_type_id' => $typeFound], ['id' => $sid]);
        }
        ...
        foreach ($this->ots as $otid => $ot) {
            foreach ($ot['cts'] as $key => $ctid) {
                if ($key === 0) {
                    $this->connection->update('opinion_type', ['consultation_type_id' => $ctid], ['id' => $otid]);
                } else {
                    $values = $this->connection->fetchAll('SELECT * from opinion_type WHERE id = ?', [$otid])[0];
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TABLE consultationtype_opiniontypes (consultationtype_id INT NOT NULL, opiniontype_id INT NOT NULL, INDEX IDX_1E2A76B8E0D2FC3D (consultationtype_id), INDEX IDX_1E2A76B835365590 (opiniontype_id), PRIMARY KEY(consultationtype_id, opiniontype_id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB');
        ...
        $this->addSql('ALTER TABLE opinion_type DROP parent_id, DROP consultation_type_id, DROP lft, DROP rgt, DROP root, DROP lvl');
    ...
    public function postDown(Schema $schema)
    {
        foreach ($this->steps as $s) {
            $ots = $this->cts[$s['type']];
            foreach ($ots as $ot) {
                $this->connection->insert('consultationstep_opiniontypes', ['consultationstep_id' => $s['step'], 'opiniontype_id' => $ot]);
            }
        }

        foreach ($this->cts as $ct => $ots) {
            foreach ($ots as $ot) {
                $this->connection->insert('consultationtype_opiniontypes', ['consultationtype_id' => $ct, 'opiniontype_id' => $ot]);
            }
        }
    }
