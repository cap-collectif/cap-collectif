...
import VotesBar from '../components/Utils/VotesBar';
            ...
            return (
              <div className="opinion__appendix" key={index}>
                <Button
                  ...
                  <i className="cap cap-arrow-67" /> {appendix.title}
                </Button>
              </div>
            );
          }
            ...
            <Col sm={12} md={8} style={{ paddingTop: '15px' }}>
              ...
              <ButtonToolbar className="opinion__votes__buttons">
                ...
                <Button
                  ...
                  disabled={false}>
                  <i className="cap cap-hand-like-2 icon-rotate" /> {'Mitigé'}
                </Button>
                <Button
                  style={{ marginLeft: 5 }}
                  bsStyle="danger"
              ...
              <div>
                ...
                <div style={{ paddingTop: '20px' }}>
                  {opinion.previewVotes &&
                    opinion.previewVotes.map((vote, index) => (
                      <OverlayTrigger
                        key={index}
                        placement="top"
                        overlay={
                          <Tooltip id={`opinion-vote-tooltip-${vote.id}`}>{vote.username}</Tooltip>
                        }>
                        <UserAvatar user={vote} style={{ marginRight: 5 }} />
                      </OverlayTrigger>
                    ))}
                </div>
                {opinion.votes.totalCount > 0 && <div>{`${opinion.votes.totalCount} votes`}</div>}
              </div>
            </Col>
        ...
        <ButtonToolbar>
          ...
          <span>
            <Button
              ...
              onClick={() => {}}
              style={{ marginLeft: 5 }}
              className="opinion__action--report btn--outline btn-dark-gray">
...
storiesOf('OpinionBox', module)
  ...
  .add('default case', () => {
    const section = {
      title: text('Title', 'Titre de la section', 'Section'),
