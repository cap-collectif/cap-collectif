...
const ProjectPreviewBody = React.createClass({
  ...
  render() {
    ...
    if (this.shouldRenderProgressBar()) {
      progress = (
        <div>
          <p style={{ marginBottom: 5 }}>Etat d'avancement</p>
          <ProjectPreviewProgressBar project={project} />
        </div>
      );
    } else {
      progress = <div style={{ height: '25px' }} />;
    }
    ...
    return (
      <div className="box project__preview__body">
        <div style={{ height: 130 }}>
          ...
          <h2 className="h4 project__preview__title smart-fade" style={{ height: 'auto', lineHeight: 'auto', margin: '5px 0' }}>
            <OverlayTrigger placement="top" overlay={tooltip}>
              <a href={link}>
                { project.title }
                {
                  externalLink &&
                    <svg style={{ marginLeft: 5, display: 'inline-block', height: 16, width: 16 }} version="1.1" id="Calque_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                      viewBox="0 0 89.7 90.1" xmlSpace="preserve">
          ...
          <Truncate lines={1} className="excerpt">{ project.author.displayName }</Truncate>
        </div>
        {progress}
      </div>
    );
