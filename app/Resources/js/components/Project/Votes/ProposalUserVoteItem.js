  ...
  render() {
    ...
    return (
      <Row className="proposals-user-votes__row" id={`vote-step${step.id}-proposal${proposal.id}`}>
        <Col className="proposals-user-votes__col" md={colWidth === 3 ? colWidth + 1 : colWidth + 2} xs={12}><a href={proposal._links.show}>{proposal.title}</a></Col>
        <Col className="proposals-user-votes__col" md={colWidth} xs={12}><i className="cap cap-user-2"></i><UserLink user={proposal.author} /></Col>
        {
          proposal.district &&
          <Col className="proposals-user-votes__col" md={colWidth} xs={12}><i className="cap cap-marker-1"></i>{proposal.district.name}</Col>
        }
