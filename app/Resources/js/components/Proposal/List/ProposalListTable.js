  ...
  getData = () => {
    ...
    return (
      ...
      proposals.edges
        ...
        .map((node) => (
          {
            title: { text: 'admin.fields.selection.proposal', value: { displayTitle: node.title && node.title, url: node.url && node.url}, width: '250px' },
            implementationPhase: { text: 'implementation-phase', value: { list: node.progressSteps && node.progressSteps, title: this.getPhaseTitle(node.progressSteps) }, width: '250px'},
            ...
            ref: { text: 'proposal.admin.reference', value: node.reference && node.reference, width: '150px'},
            district: { text: 'proposal.district', value: node.district && node.district.name, hidden: !step.form.usingDistrict },
            category: { text: 'proposal.category', value: node.category && node.category.name, hidden: !step.form.usingCategories },
            theme: { text: 'proposal.theme', value: node.theme && node.theme.title, hidden: !step.form.usingThemes },
            priceEstimation: { text: 'proposal.estimation', value: node.estimation && node.estimation },
            likers: { text: 'project_download.label.likers', value: node.likers && node.likers, width: '250px'},
            lastActivity: {
              text: 'last-activity',
              value: {
            ...
            publishedOn: { text: 'published-on', value: node.createdAt && node.createdAt, width: '150px'},
          }
        ))
    )
  };
    ...
    return (
      <ListGroup className="list-group-custom">
        {this.getData().map(item => {
          const list = item.implementationPhase.value && item.implementationPhase.value.list.map(e => {
            let isActive = false;
