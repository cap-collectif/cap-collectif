...
export const ProposalVoteBasketWidget = React.createClass({
  ...
  render() {
    ...
    const selectedStep = votableSteps.filter(
...
const mapStateToProps = state => {
  return {
    userVotesCountByStepId: mapValues(
      state.proposal.userVotesByStepId,
      votes => votes.length,
    ),
    creditsLeftByStepId: state.proposal.creditsLeftByStepId,
    votableSteps: state.project.projectsById[
      state.project.currentProjectById
    ].steps.filter(step => step.votable),
