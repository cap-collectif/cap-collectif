...
export type ConditionsFormatted = Record<string, ConditionFormatted>
...
const formatConditionToIdJump = (
  ...
  const result = jumps.map((jump: JumpFormatted) => {
    responses.forEach((response: FormattedResponse) => {
      ...
      const questionToValidateCurrentCondition = jump.questionsToValidate[indexQuestionToValidateCurrentCondition]
      const isRightAnswer =
