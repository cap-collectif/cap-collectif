import type { Questions, ResponsesInReduxForm, SubmitResponses, Media } from '~/components/Form/Form.type'
...
export const formatSubmitResponses = (responses: ResponsesInReduxForm, questions: Questions): SubmitResponses => {
  if (!responses) return []
  ...
  return responses.map(res => {
    ...
    if (questionType === 'medias') {
      const mediaResponses = res.value as any as ReadonlyArray<Media>
    ...
    if (questionType === 'select') {
      ...
      return {
        question: res.question,
        value: value ? (value as any as ReactSelectValue).value : null,
      }
    }
