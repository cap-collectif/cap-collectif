  ...
  questions.map(({ id, title, alwaysJumpDestinationQuestion, jumps }) => {
    ...
    if (jumps && jumps.length > 0) {
      ...
      jumps.forEach(jump => {
        ...
        if (conditionsOfJumpQuestion.isValidated) {
          ...
          if (question.questionsToDisplay.length === 0) {
            ...
            question.questionsNotDisplay = [...question.questionsNotDisplay, conditionsOfJumpQuestion.destination]
          }
        } else {
      ...
      if (alwaysJumpDestinationQuestion && !hasOneJumpValidated && hasResponse) {
        question.questionsToDisplay = [alwaysJumpDestinationQuestion.id]
      } else if (alwaysJumpDestinationQuestion && (hasOneJumpValidated || (!hasResponse && !hasOneJumpValidated))) {
        question.questionsNotDisplay = [...question.questionsNotDisplay, alwaysJumpDestinationQuestion.id]
      }
    }
