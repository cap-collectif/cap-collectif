...
describe('csvToArray', () => {
  ...
  it('should remove empty lines when setting the option `removeEmptyLines`', () => {
    const result = csvToArray(CSV_CONTENT_EMPTY_LINES, { removeEmptyLines: true });
    expect(result).toStrictEqual(['rem@chan.com', 'ram@chan.com', 'another one']);
  });
  ...
  it('should strip the first row based on the `stripFirstRow` option', () => {
    ...
    expect(result).toStrictEqual(['ram@chan.com', 'another one']);
  });
});
