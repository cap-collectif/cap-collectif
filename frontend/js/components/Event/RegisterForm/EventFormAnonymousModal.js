...
export const EventFormAnonymousModal = ({ eventId, register }: Props) => {
  ...
  const onSubmit = (values: FormValues, hide: () => void) => {
    ...
    return (
      SubscribeToEventAsNonRegisteredMutation.commit({
        input,
      })
        .then(() => {
          toast({
            variant: 'success',
            content: intl.formatHTMLMessage({ id: 'event_registration.create.register_success' }),
          });
          reset();
          register();
          hide();
        ...
        .catch(e => {
          setError('email', {
            type: 'server',
            message: intl.formatHTMLMessage({ id: 'user-already-registered-with-this-email' }),
          });
        })
    );
  };
    ...
    <Modal
      ...
      {({ hide }) => (
        <>
          <ResetCss>
            <Modal.Header>
              <Heading>{intl.formatMessage({ id: 'subscribe-event' })}</Heading>
            </Modal.Header>
          </ResetCss>
          <Modal.Body>
            <FormControl name="username" control={control} isRequired>
              <FormLabel
                ...
                label={intl.formatMessage({ id: 'form.label_username' })}
              />
              <FieldInput
                id="EventFormAnonymousModal-username"
                name="username"
                control={control}
                type="text"
              />
            ...
            <FormControl name="email" control={control} isRequired>
              ...
              />
              <FieldInput
                id="EventFormAnonymousModal-email"
                name="email"
                control={control}
                type="email"
              />
            ...
            <FormControl name="accept" control={control} isRequired>
              <FieldInput
                id="EventFormAnonymousModal-accept"
                name="accept"
                control={control}
                type="checkbox">
