...
const NotifyInfoMessage: StyledComponent<any, {}, HTMLDivElement> = styled.div`
  ...
  opacity: 0.8;
`
export const EventEditModal = ({ submitting, dispatch, show, pristine, query, event, handleClose }: Props) => {
  const intl = useIntl()
  return (
    <Modal show={show} onClose={handleClose} size={CapUIModalSize.Xl} aria-labelledby="contained-modal-title-lg">
      <ResetCss>
        <Modal.Header>
          <Heading as="h4">
            {intl.formatMessage({
      ...
      <Modal.Footer>
        {event && event.participants.totalCount > 0 && (
          <NotifyInfoMessage>
            {intl.formatMessage({
              id: 'event-modification-notification-to-members',
            })}
          </NotifyInfoMessage>
        )}
        <Flex>
          <Button variant="secondary" variantSize="big" variantColor="primary" mr={2} onClick={handleClose}>
            {intl.formatMessage({
              id: 'global.cancel',
            })}
          </Button>
