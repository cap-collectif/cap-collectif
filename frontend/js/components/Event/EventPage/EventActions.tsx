...
type Props = {
  readonly queryRef: EventActions_query$key | null | undefined
}
...
export const EventActions = ({ queryRef }: Props) => {
      ...
      <>
        ...
        {displayLeftEventPlacesTag ? (
          <Text as="span" mt="12px">
            {intl.formatMessage(
              {
                id: 'left-event-places',
              },
              {
                num: availableRegistration,
              },
            )}
          </Text>
        ) : null}
  ...
  const eventIsNotApproved = viewerDidAuthor && hasProposeEvent && review && review?.status !== 'APPROVED'
  return (
