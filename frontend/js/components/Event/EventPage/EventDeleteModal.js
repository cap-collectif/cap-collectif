...
const onDelete = (eventId: string, intl: IntlShape) => {
  return DeleteEventMutation.commit({
    ...
    .then(() => {
      toast({
        variant: 'success',
        content: intl.formatMessage({ id: 'event-deleted' }),
      });
          ...
          <ResetCss>
            <Modal.Header>
              <Heading as="h4" color="blue.900">
                {intl.formatMessage({ id: 'event.alert.delete' })}
              </Heading>
            </Modal.Header>
          </ResetCss>
          <Modal.Body>
            <Text>{intl.formatMessage({ id: 'notification-send-to-all-members' })}</Text>
            <Checkbox
              checked={confirmed}
