...
export const Aside = ({ children }: {| children: React.Node |}) => (
  <Flex
    direction="column"
...
export const Content = ({ children }: {| children: React.Node |}) => {
...
export const About = ({ children }: {| children?: React.Node | string |}) => {
  const intl = useIntl();
  if (!children) return null;
  return (
    <>
      <Text as="span" fontWeight={600} fontSize={5} lineHeight="initial" mb={4}>
        {intl.formatMessage({ id: 'about-that-event' })}
      </Text>
      {typeof children === 'string' ? (
        <WYSIWYGRender className="description" value={children} />
      ) : (
        children
      )}
    </>
...
const renderDate = (startAt: string, endAt: ?string, intl: IntlShape) => {
...
export const EventPageContent = ({ queryRef }: Props) => {
  ...
  return (
    <Flex
      ...
      alignItems="center"
      px={[4, 0]}
      bg={['neutral-gray.50', 'white']}>
      <Content>
        <Flex flexDirection="column" flex="1">
          {event.viewerDidAuthor && hasProposeEventEnabled && (
        ...
        <Flex direction="column" mt={[8, 0]}>
          <Aside>
            {timeRange?.startAt
              ? renderDate(
                  timeRange.startAt.replace(/-/g, '/'),
                  timeRange.endAt?.replace(/-/g, '/'),
                  intl,
                )
              : null}
            {googleMapsAddress && (
