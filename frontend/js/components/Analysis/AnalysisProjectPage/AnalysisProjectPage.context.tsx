...
import type { Action, AnalysisProjectPageParameters } from './AnalysisProjectPage.reducer'
import { createReducer, DEFAULT_FILTERS, DEFAULT_SORT, DEFAULT_STATUS } from './AnalysisProjectPage.reducer'
...
export type Context = {
  ...
  readonly parameters: AnalysisProjectPageParameters
...
export const useAnalysisProposalsContext = (): Context => {
  ...
  if (!context) {
    throw new Error(`You can't use the useAnalysisProposalsContext outside a AnalysisProposalsProvider component.`)
  }
...
export const AnalysisProposalsProvider = ({ children }: ProviderProps) => {
  ...
  return <AnalysisProjectPageContext.Provider value={context}>{children}</AnalysisProjectPageContext.Provider>
}
