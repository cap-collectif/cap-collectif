...
const UserAnalystListHidden = ({ proposal, max }: Props) => {
  ...
  return (
    <UserAnalystListHiddenContainer>
      ...
      />
      <Overlay show={isTooltipRestAnalystsDisplay} target={refTooltipRestAnalysts.current} placement="top">
            ...
            {restAnalysts.map(analyst => (
              <UserAvatarWrapper key={`analyst-${analyst.id}`}>
                <UserAvatar
                  ...
                  badge={getBadge(getStatusAnalyst(analyses, analyst.id, decisionState, assessmentState), true)}
                />
