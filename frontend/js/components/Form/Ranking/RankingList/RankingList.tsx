...
type RankingListProps = {
  id: string
  onChange: (arg0: Array<Field>) => void
...
type ListItems = Record<$Values<typeof ID_LIST>, Array<Field>>
...
const RankingList = ({ dataForm, isDisabled, onChange, id }: RankingListProps) => {
  ...
  const moveWithoutDrag = (currentIndex, from: $Values<typeof ID_LIST>, to: $Values<typeof ID_LIST>) => {
  ...
  return (
    <RankingListContainer ariaLabelledby={`label-${id}`} tabIndex={0} role="group">
      <VisuallyHidden id="drag-instructions">
        {intl.formatMessage({
          id: 'aria-ranking-item-instructions',
        })}
      </VisuallyHidden>
