...
type Props = {
  ...
  readonly switchConfirm?: boolean | null | undefined
  readonly setSwitchConfirm: () => void
  readonly switchToNewEditor: () => void
}

const EditorSwitchModal = ({ show, setShow, switchConfirm, setSwitchConfirm, switchToNewEditor }: Props) => {
  const intl = useIntl()
  return (
    <>
      <Button
          ...
          <>
            <Modal.Header>
              <Heading as="h4" color="blue.900">
                {intl.formatMessage({
                  id: 'are-you-sure-activating-new-editor',
                })}
              </Heading>
            </Modal.Header>
            <Modal.Body>
              <Text>
                {intl.formatMessage({
                  id: 'warning-activate-new-editor',
                })}
              </Text>
              <Text
                fontWeight={600}
            ...
            <Modal.Footer spacing={2}>
              <Button variantSize="big" variant="secondary" variantColor="hierarchy" onClick={() => setShow(false)}>
                {intl.formatMessage({
                  id: 'global.cancel',
                })}
              </Button>
