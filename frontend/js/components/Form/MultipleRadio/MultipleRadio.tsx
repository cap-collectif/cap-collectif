...
const MultipleRadio = ({ disabled = false, id, field, value = {}, onBlur, onChange, typeForm }: Props) => {
  ...
  return (
    <div className="form-group form-fields" role="radiogroup" aria-labelledby={`label-${id}`} id={id}>
      {field.helpText && (
      ...
      {field.choices &&
        field.choices.map(choice => {
          ...
          return (
            <ItemMultipleRadioContainer key={choiceKey} hasImage={choice.image && !!choice.image.url}>
              <Radio
              ...
              {choice.description && (
                <WYSIWYGRender className="mb-20 pl-20 choice-description" value={choice.description} tagName="i" />
              )}
