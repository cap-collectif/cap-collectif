...
export class ImageUpload extends React.Component<Props, State> {
  ...
  removeMedia = (media: Record<string, any>) => {
    const { onChange, multiple, value } = this.props
                ...
                <div {...getRootProps()} className="image-uploader__dropzone-label">
                  ...
                  >
                    <Button className="image-uploader__btn">
                      ...
                      <FormattedMessage
                        id={multiple ? 'global.image_uploader.file.btn' : 'global.form.ranking.select'}
                      />
                    </Button>
                  </p>
                </div>
          ...
          <Col xs={12} sm={12}>
            ...
            <div className="image-uploader__preview text-center">
              {value && Array.isArray(value) ? (
                value.map(media => <Image alt="" key={media.id} src={media.url} className="img-responsive" />)
              ) : (
        ...
        {error && error.length > 0 && (
          <Col xs={12} sm={12}>
            <AppBox mt={4} color={colors.dangerColor} className="wrapper-error">
              <Icon name={ICON_NAME.ERROR} color={colors.dangerColor} size={ICON_SIZE.SM} verticalAlign="-3px" />
              &nbsp;
              <FormattedMessage
                id={error}
                values={{
