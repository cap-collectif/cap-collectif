...
const MultipleCheckbox = ({ disabled = false, id, field, value = {}, onBlur, onChange, typeForm }: Props) => {
    ...
    <div className="form-group form-fields" id={id} role="group" aria-labelledby={`label-${id}`}>
      ...
      {field.choices &&
        field.choices.map(choice => {
          ...
          return (
            <ItemMultipleCheckboxContainer key={choiceKey} hasImage={choice.image && !!choice.image.url}>
              <Checkbox
                name={fieldName}
                id={`${id}_${choiceKey}`}
                value={choiceValue}
