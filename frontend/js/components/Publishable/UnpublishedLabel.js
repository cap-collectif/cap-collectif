...
`;
const StyledPopover: StyledComponent<{}, {}, typeof Popover> = styled(Popover)`
  button {
...
const UnpublishedLabel = ({ publishable: publishableRef }: Props) => {
  ...
  const intl = useIntl();
  const viewer: { email: string } = useSelector(state => state.user.user);
  ...
  if (publishable.notPublishedReason === 'WAITING_AUTHOR_CONFIRMATION') {
    popoverContent = (
      <>
        <Popover.Header>
          <Text fontSize={3} fontWeight="bold" className="excerpt_dark">
            {intl.formatMessage({ id: 'account-pending-confirmation' })}
          </Text>
        </Popover.Header>
        <Popover.Body marginBottom={0}>
          <Text fontSize={3}>
            {intl.formatMessage(
              ...
              { contentType: 'contribution', emailAddress: viewer.email },
            )}
          </Text>
  ...
  if (publishable.notPublishedReason === 'AUTHOR_NOT_CONFIRMED') {
    popoverContent = (
      <>
        <Popover.Header>
          <Text fontSize={3} fontWeight="bold" className="excerpt_dark">
            {intl.formatMessage({ id: 'account-not-confirmed-in-time' })}
          </Text>
        </Popover.Header>
        <Popover.Body marginBottom={0}>
          <Text fontSize={3}>
  ...
  if (publishable.notPublishedReason === 'AUTHOR_CONFIRMED_TOO_LATE') {
    popoverContent = (
      <>
        <Popover.Header>
          <Text fontSize={3} fontWeight="bold" className="excerpt_dark">
            {intl.formatMessage({ id: 'account-confirmed-too-late' })}
          </Text>
        </Popover.Header>
        <Popover.Body marginBottom={0}>
          <Text fontSize={3}>
