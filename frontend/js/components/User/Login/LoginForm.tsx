...
type ReduxProps = {
  readonly restrictConnection: boolean
...
const validate = (values: { username: string; password: string; new_password: string }) => {
  ...
  if (!values.username || values.username.length < 1 || !values.new_password || values.new_password.length < 1) {
...
export default connect<any, any>(mapStateToProps)(container)
