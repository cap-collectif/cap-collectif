...
describe('<LoginSocialButtons />', () => {
  ...
  it('renders only FranceConnect button', () => {
    ...
    const wrapper = shallow(<LoginSocialButtons {...propsWithFeatureLoginFranceConnectActivated} />)
    expect(wrapper).toMatchSnapshot()
  })
  ...
  it('renders without OR separator', () => {
    enableFeatureFlags(['login_facebook', 'login_saml', 'login_franceconnect', 'login_cas', 'sso_by_pass_auth'])
    const wrapper = shallow(<LoginSocialButtons {...propsWithAllLoginFeaturesLoginActivatedAndORSeparatorDisabled} />)
    expect(wrapper).toMatchSnapshot()
  })
})
