...
import CheckCircle from '@shared/ui/LegacyIcons/CheckCircle'
...
type Props = {
  ...
  readonly isPasswordFocus: boolean
  readonly passwordComplexityScore: number
  readonly passwordConditions: {
    length: boolean
...
`
export const getMatchingPasswordError = (passwordFieldName: string, passwordConditions: Record<string, any>) => {
  let error = null
  const errorObject = {}
  const sum =
...
const asyncPasswordValidateLogic = (
  ...
  return new Promise((resolve, reject) => {
    fetchQuery_DEPRECATED(environment, getPasswordComplexityScore, credentialValues).then(res => {
      dispatch(
        change(formName, 'passwordComplexityScore', res.passwordComplexityScore + (passwordConditions.length ? 1 : 0)),
      )
    })
...
export class UserPasswordComplexityField extends Component<Props> {
  ...
  renderPasswordInformation(
    ...
    passwordConditions: Record<string, any>,
    ...
    return (
      <StyleContainer>
        ...
        >
          ...
          <div className={`mb-5 mt-5 ${passwordConditions && passwordConditions.length ? 'text-green' : 'text-gray'}`}>
            <CheckCircle color={passwordConditions && passwordConditions.length ? 'green' : '#a7a0a0'} />
          ...
          >
            <CheckCircle color={passwordConditions && passwordConditions.upperLowercase ? 'green' : '#a7a0a0'} />
            <FormattedMessage id="lower-and-upper-case-letters" />
          </div>

          <div className={`mb-5 mt-5 ${passwordConditions && passwordConditions.digit ? 'text-green' : 'text-gray'}`}>
            <CheckCircle color={passwordConditions && passwordConditions.digit ? 'green' : '#a7a0a0'} />
            <FormattedMessage id="at-least-1-digit" />
          </div>

          <div>
            <FormattedMessage id="password.security" />{' '}
            <FormattedMessage id={this.getMatchingPasswordSecurityAttributes(passwordComplexityScore).text} />
          </div>
          <ProgressBar
  ...
  render() {
    const { field, passwordComplexityScore, passwordConditions, formAsyncErrors, name, isPasswordFocus, formName } =
      this.props
    ...
    return (
      <Popover trigger={['click']} keepOnHover useArrow placement="right" className="in" id="pinned-label">
        <Popover.Trigger>
          <div>{field}</div>
        </Popover.Trigger>
        {isPasswordFocus && (
