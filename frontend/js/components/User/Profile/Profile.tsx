...
import '~relay/Profile_viewer.graphql'
...
const validate = (values: Record<string, any>) => {
  const errors: any = {}
  const fields = ['biography', 'neighborhood', 'linkedInUrl', 'twitterUrl', 'instagramUrl', 'facebookUrl', 'username']
...
const onSubmit = (values: Record<string, any>, dispatch: Dispatch, props: Props) => {
  ...
  const profilePageIndexed =
    typeof values.doNotIndexProfile !== 'undefined' ? !values.doNotIndexProfile : props.viewer.profilePageIndexed
              ...
              {organizations.map(org => (
                ...
                >
                  <Flex
                    ...
                    color="gray.900"
                  >
                    {org?.media?.url ? <Avatar size="tiny" alt={org.username || ''} src={org.media.url || ''} /> : null}
                    <Text>{org?.username}</Text>
                  </Flex>
                  <LeaveOrganizationModal organizationName={org?.username || ''} organizationId={org?.id || ''} />
                </Flex>
              ))}
            </>
          ) : null}
