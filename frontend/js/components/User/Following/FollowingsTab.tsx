...
import '~relay/FollowingsTab_viewer.graphql'
...
export class FollowingsTab extends Component<Props, State> {
  ...
  onUnfollowAll() {
    ...
    const idsProposal =
      viewer.followingProposals && viewer.followingProposals.edges && viewer.followingProposals.edges.length > 0
        ? viewer.followingProposals.edges.filter(Boolean).map(edge => edge.node.id)
        : null
    const idsOpinion =
      viewer.followingOpinions && viewer.followingOpinions.edges && viewer.followingOpinions.edges.length > 0
              ...
              <div id="all-projects">
                {Object.keys(projectsById).map((project, id) => {
                  if (projectsById[project].type === 'opinionProject') {
                    return <OpinionProjectRow key={id} project={projectsById[project].object} viewer={viewer} />
                  }

                  return <ProposalProjectRow key={id} project={projectsById[project].object} viewer={viewer} />
                })}
              </div>
            </Collapse>
          ) : (
