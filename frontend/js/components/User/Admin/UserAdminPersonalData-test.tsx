...
describe('<UserAdminPersonalData/>', () => {
  ...
  const userIsConfirmed = { ...user, isEmailConfirmed: true }
  const userIsNotConfirmed = { ...user, isEmailConfirmed: false }
  it('should render when user is confirmed by email and viewer is super admin', () => {
    const wrapper = shallow(
      <UserAdminPersonalData {...props1} user={userIsConfirmed} viewer={viewer} isViewer={false} isSuperAdmin />,
    )
