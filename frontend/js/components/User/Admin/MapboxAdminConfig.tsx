...
const onSubmit = async (values: FormValues) => {
  const input = { ...values, provider: 'MAPBOX' }

  try {
    await ChangeMapProviderTokenMutation.commit({
      // @ts-ignore
      input,
    })
  } catch (e) {
...
export const MapboxAdminConfig = (props: Props) => {
  ...
  return (
    <form className="mapbox__config" onSubmit={handleSubmit}>
      ...
      >
        <SubmitButtonInner>
          {submitting ? <Loader show inline size={20} color="white" /> : <FormattedMessage id="verify" />}
        </SubmitButtonInner>
      </Button>
      <AlertForm
        valid={valid}
