...
type Props = {
  ...
  readonly viewer: DesktopDebateStepPageArguments_viewer | null | undefined
}
          ...
          >
            <Menu.List>
              <Menu.OptionGroup
                ...
                type="radio"
                onChange={newValue => {
                  setFilter(newValue as any as Filter)
                  const field = newValue === 'MOST_SUPPORTED' ? 'VOTE_COUNT' : 'PUBLISHED_AT'
                  const direction = newValue === 'MOST_SUPPORTED' ? 'DESC' : newValue
                  if (yesState)
                    yesState.refetchConnection(CONNECTION_CONFIG_YES, CONNECTION_NODES_PER_PAGE, null, {
                      orderBy: {
                        field,
                        direction,
                      },
      ...
      <Flex direction="row" spacing={6}>
        <Flex direction="column" flex={1}>
          <DebateStepPageArgumentsPagination
            viewerUnpublishedArgument={viewerUnpublishedArgument?.type === 'FOR' ? viewerUnpublishedArgument : null}
        ...
        <Flex direction="column" flex={1}>
          <DebateStepPageArgumentsPagination
            viewerUnpublishedArgument={viewerUnpublishedArgument?.type === 'AGAINST' ? viewerUnpublishedArgument : null}
...
export default createFragmentContainer(DesktopDebateStepPageArguments, {
  ...
  `,
}) as RelayFragmentContainer<typeof DesktopDebateStepPageArguments>
