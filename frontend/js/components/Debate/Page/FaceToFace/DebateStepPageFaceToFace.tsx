...
type Props = {
  readonly step: DebateStepPageFaceToFace_step | null | undefined
...
export const DebateStepPageFaceToFace = ({ step, isMobile }: Props): Node => {
  ...
  const hasMore = !isMobile && ((againstOpinion?.body.length || 0) > 500 || (forOpinion?.body.length || 0) > 500)
  return (
        ...
        {forOpinion && againstOpinion ? (
          <>
            <Flex direction={['column', 'row']} spacing={6}>
              <DebateOpinion isMobile={isMobile} opinion={forOpinion} readMore={readMore || !hasMore} />
              <DebateOpinion isMobile={isMobile} opinion={againstOpinion} readMore={readMore || !hasMore} />
            </Flex>
            {hasMore && (
...
export default createFragmentContainer(DebateStepPageFaceToFace, {
  step: graphql`
    fragment DebateStepPageFaceToFace_step on DebateStep @argumentDefinitions(isMobile: { type: "Boolean!" }) {
      debate {
        opinions {
          edges {
            node {
  ...
  `,
}) as RelayFragmentContainer<typeof DebateStepPageFaceToFace>
