...
describe('<DebateStepPageLinkedArticlesDrawer />', () => {
        ...
        edges: [
          {
            node: {
              ...
              url:
                'https://www.generationlibre.eu/medias/non-au-passeport-vaccinal-gaspard-koenig/',
            },
          },
          {
            node: {
              ...
              title:
                'Macron: ceux qui disent "on aura un vaccin en mars ou avril prochain, vous trompent"',
              url:
                'https://actu.orange.fr/societe/videos/macron-ceux-qui-disent-on-aura-un-vaccin-en-mars-ou-avril-prochain-vous-trompent-CNT000001tKjH0.html',
            },
          },
          {
            node: {
  ...
  it('should render nothing when empty', async () => {
    ...
    environment.mock.queueOperationResolver(operation =>
      MockPayloadGenerator.generate(operation, emptyMockResolvers),
    );
  ...
  it('should render nothing when API could not load articles', async () => {
    ...
    environment.mock.queueOperationResolver(operation =>
      MockPayloadGenerator.generate(operation, erroredMockResolvers),
    );
    testComponentTree = ReactTestRenderer.create(
      <TestComponent isOpen={false} onClose={onClose} />,
    );
