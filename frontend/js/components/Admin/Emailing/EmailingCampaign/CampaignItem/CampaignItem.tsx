...
export const CampaignItem = ({ campaign, selected }: Props) => {
  const { id, name, mailingList, sendAt, status, mailingInternal, emailingGroup, project } = campaign
  const intl = useIntl()
  return (
    <Container rowId={id} selected={selected}>
      ...
      <InfoRow>
        <p>
          ...
          <span className="label-status">
            {intl.formatMessage({
              id: STATUS_CAMPAIGN[status].label,
            })}
          </span>
        </p>
      ...
      {(mailingList || mailingInternal || emailingGroup || project) && (
