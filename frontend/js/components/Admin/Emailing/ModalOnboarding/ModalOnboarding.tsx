...
const ModalOnboarding = ({ isOnlyProjectAdmin }: Props) => {
  ...
  return (
    <ModalContainer animation={false} show={isOpen} onHide={onClose} bsSize="small" aria-labelledby="modal-title">
      <Flex direction="column" p={8}>
        <Flex direction="row" justify="space-between">
          <Tag variant="yellow">
            {intl.formatMessage({
              id: 'global.beta.feature',
            })}
          </Tag>
        ...
        <AppBox as={Modal.Body} p={0} my={4}>
          <Flex direction="column">
            ...
            <Heading as="h4" mb={4} fontWeight={FontWeight.Semibold} color="gray.900">
              {intl.formatMessage({
                id: 'welcome.new.emailing.tool',
              })}
            </Heading>
            <Text color="gray.700" mb={4}>
              {intl.formatMessage({
                id: announcementTranslationKey,
              })}
            </Text>
            <Text color="gray.700">
              {intl.formatMessage({
                id: 'consult-help-center-new-feature',
              })}
            </Text>
          </Flex>
        </AppBox>
