...
const ModalAddDomain = ({ dispatch }: Props): React.Node => {
  const intl = useIntl();
  const service = useSelector((state: GlobalState) =>
    formValueSelector(formName)(state, 'domain-service'),
  );

  return (
    <Flex direction="column" spacing={8}>
      <InfoMessage variant="warning">
        <InfoMessage.Title>{intl.formatMessage({ id: 'global.prerequisite' })}</InfoMessage.Title>
        <InfoMessage.Content>
          {intl.formatMessage({ id: 'get-access-to-customer-area-dns-supplier' })}
        </InfoMessage.Content>
      </InfoMessage>

      <Flex direction="column">
        <Label>{intl.formatMessage({ id: 'which-kind-domain-service-added' })}</Label>
      ...
      <Field
        type="text"
        label={intl.formatMessage({ id: 'inform-domain-to-authenticate' })}
        id="domain"
        name="domain"
        component={component}
