...
type Values = {|
  +domain: string,
  +'domain-service': SenderEmailDomainService,
|};

const onSubmit = (values: Values, dispatch: Dispatch, props: Props) => {
  const input = {
    service: ((values['domain-service'].toUpperCase(): any): SenderEmailDomainService),
    value: values.domain,
  };

  return CreateSenderEmailDomainMutation.commit({ input })
    .then(response => {
      ...
      if (response.createSenderEmailDomain?.errorCode) {
        ...
        toast({
          variant: 'success',
          content: props.intl.formatMessage({ id: 'domain-successfully-added' }),
        });
      }
    })
    .catch(() => {
...
const ModalsAddDomain = ({ dispatch, domain, service, intl }: Props): React.Node => {
  return (
    <ModalSteps
      ariaLabel={intl.formatMessage({ id: 'authentication-sender-domains' })}
      disclosure={
        <Button
          variant="secondary"
        ...
        <>
          <ModalSteps.Header>
            <ModalSteps.Header.Label>
              {intl.formatMessage({ id: 'authentication-sender-domains' })}
            </ModalSteps.Header.Label>
          </ModalSteps.Header>
          ...
          <ModalSteps.Body>
            <ModalAddDomain
              id="add-domain"
              label={intl.formatMessage({ id: 'add-a-domain' })}
              validationLabel={intl.formatMessage({ id: 'dns-supplier' })}
              dispatch={dispatch}
            />
            <ModalDNSSupplier
              id="dns-supplier"
              label={intl.formatMessage({ id: 'dns-supplier' })}
              validationLabel={intl.formatMessage({ id: 'dns-registration' })}
            />
            <ModalDNSRegistration
              id="dns-registration"
              label={intl.formatMessage({ id: 'update-dns-registration' })}
              validationLabel={intl.formatMessage({ id: 'global.validate' })}
              dkim={{
                subDomain: domain
                  ? service === 'MAILJET'
...
const ModalsAddDomainForm = (reduxForm({
  form: formName,
  onSubmit,
})(ModalsAddDomain): React.AbstractComponent<{||}>);
...
const ModalsAddDomainFormConnected = (connect<any, any, _, _, _, _>(mapStateToProps)(
  ModalsAddDomainForm,
): React.AbstractComponent<BeforeProps>);
