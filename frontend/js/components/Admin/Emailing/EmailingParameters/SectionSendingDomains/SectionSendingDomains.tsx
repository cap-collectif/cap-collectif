...
type Props = {
  readonly senderEmailDomains: SectionSendingDomains_senderEmailDomains$key
  readonly senderEmails: SectionSendingDomains_senderEmails$key
}
    ...
    <Section direction="row" align="stretch" justify="space-between" spacing={4}>
      <Flex direction="column" width="30%">
        <Flex direction="column" mb={7} spacing={2}>
          <Section.Title>
            {intl.formatMessage({
              id: 'authentication-sender-domains-dkim-spf',
            })}
          </Section.Title>
          <Section.Description>
            {intl.formatMessage({
              id: 'authentication-sender-domains-explanation',
            })}{' '}
            <Link
        ...
        <Table.Tbody>
          {senderEmailDomains.map(senderEmailDomain => (
            <Table.Tr rowId={senderEmailDomain.id} key={senderEmailDomain.id}>
              ...
              <Table.Td>
                {!senderEmails.some(senderEmail => senderEmail.address.includes(senderEmailDomain.value)) && (
