...
describe('<SectionEmailingService />', () => {
  ...
  const query = graphql`
    query SectionEmailingServiceTestQuery($type: ExternalServiceConfigurationType!)
  ...
  beforeEach(() => {
    ...
    const TestRenderer = ({ componentProps, queryVariables: variables }) => {
      ...
      return (
        <SectionEmailingService
          externalServiceConfiguration={data.externalServiceConfiguration}
          {...componentProps}
        />
      );
    };
