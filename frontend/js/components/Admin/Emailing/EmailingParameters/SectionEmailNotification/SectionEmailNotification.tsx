...
type Props = BeforeProps & ReduxFormFormProps
      ...
      .then(response => {
        if (response.updateSiteParameter?.errorCode === 'INVALID_VALUE') {
          toast({
            variant: 'danger',
            content: intl.formatMessage({
              id: 'global.constraints.email.invalid',
            }),
          })
...
const SectionEmailNotification = ({ query: queryFragment }: Props): JSX.Element => {
  const query = useFragment(FRAGMENT, queryFragment)
  const { senderEmails, senderEmailDomains } = query
  const intl = useIntl()
  const defaultEmailDomain =
    senderEmailDomains.find(senderEmailDomain => senderEmailDomain.spfValidation && senderEmailDomain.dkimValidation)
      ?.value || ''
  return (
    <Section direction="row" align="stretch" justify="space-between" spacing={4}>
      <Flex direction="column" width="30%" spacing={2}>
        ...
        <Section.Description>
          {intl.formatMessage({
            id: 'notification-email-explanation',
          })}
        </Section.Description>
      </Flex>
