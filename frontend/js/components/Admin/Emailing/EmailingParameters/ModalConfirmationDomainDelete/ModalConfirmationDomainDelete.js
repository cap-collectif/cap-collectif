...
type Props = {|
  +senderEmailDomain: ModalConfirmationDomainDelete_senderEmailDomain$key,
|};
...
const deleteSenderEmailDomain = (
  ...
  return DeleteSenderEmailDomainMutation.commit({ input })
    .then(response => {
      if (response.deleteSenderEmailDomain?.errorCode) {
        ...
        toast({
          variant: 'success',
          content: intl.formatMessage({ id: 'domain-successfully-deleted' }),
        });
      }
    })
    .catch(() => {
...
const ModalConfirmationDomainDelete = ({
  senderEmailDomain: senderEmailDomainFragment,
}: Props): React.Node => {
