...
type BeforeProps = {|
  +senderEmailDomains: ModalAddEmailSender_senderEmailDomains$key,
  +intl: IntlShape,
  +initialValues: {|
    'email-domain': string,
  |},
...
const onSubmit = (values: Values, dispatch: Dispatch, props: Props) => {
  return CreateSenderEmailMutation.commit({
    ...
    .then(response => {
      ...
      toast({
        variant: 'success',
        content: props.intl.formatMessage({ id: 'sender-email-successfully-added' }),
      });
    })
    .catch(() => {
      dispatch(reset(formName));
            ...
            <FieldContainer direction="row" align="flex-end">
              ...
              <Field
                ...
                options={uniqBy(senderEmailDomains, 'value')
                  .filter(
                    senderEmailDomain =>
                      senderEmailDomain.spfValidation && senderEmailDomain.dkimValidation,
                  )
...
const ModalAddEmailSenderForm = (reduxForm({
  form: formName,
  onSubmit,
})(ModalAddEmailSender): React.AbstractComponent<BeforeProps>);
