...
type Error = {
  id: string
  values?: {
    fieldName: JSX.Element | JSX.Element[] | string
  }
}
type ReduxProps = {
  title: string
            ...
            {showError && (
              <ErrorContainer>
                <p>
                  ...
                  {intl.formatMessage({
                    id: 'admin.fields.question_choice.required',
                  })}
                </p>

                <div>
                  {(Object.values(errors) as any as Error[]).map((error, idx) => (
                    <p key={idx}>
                      <span>{`${idx + 1}. `}</span>
                      ...
                      {error.values ? (
                        <FormattedMessage
                          id={error.id}
                          values={{
                            fieldName: error.values.fieldName,
                          }}
                        />
                      ) : (
