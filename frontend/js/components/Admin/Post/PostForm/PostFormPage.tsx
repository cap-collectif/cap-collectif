...
type Props = {
  readonly queryReference: PreloadedQuery<PostFormPageQueryType>
...
const PostFormPage = ({ queryReference, postId, isAdmin }: Props): JSX.Element => {
    ...
    return {
      ...
      theme:
        themes && themes.length > 0
          ...
          : null, // @ts-ignore
      proposal: proposal && proposal.length > 0 && proposal[0]?.title ? proposal[0]?.title : null, // @ts-ignore
  ...
  return (
    // @ts-ignore
    <React.Suspense>
      <PostForm initialValues={getInitialValues()} query={query} intl={intl} isAdmin={isAdmin} locales={locales} />
    </React.Suspense>
  )
}
