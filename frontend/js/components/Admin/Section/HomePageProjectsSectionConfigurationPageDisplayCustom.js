...
function HomePageProjectsSectionConfigurationPageDisplayCustom({
  ...
  useEffect(() => {
    if (projects.length >= maxProjectsDisplay) {
      setError(intl.formatMessage({ id: 'nb.projects.limit.reached' }));
      setIsDisabled(true);
      return;
    }
    setError(null);
  ...
  return (
    <>
      <Select
        ...
        placeholder={intl.formatMessage({ id: 'search.by.project.title' })}
        noOptionsMessage={() => intl.formatMessage({ id: 'search.no.project.found' })}
      />
