...
const ProjectAdminMergeModaleStyle: StyledComponent<{}, {}, HTMLDivElement> = styled.div`
  ul {
    list-style: none;
    padding-left: 0;
  }
...
const onSubmit = (values: FormValues, dispatch: Dispatch, props: Props) => {
  ...
  return CreateProposalFusionMutation.commit({ input })
    .then(response => {
      if (!response.createProposalFusion) {
        throw new Error('Mutation "CreateProposalFusionMutation" failed.');
      }
      if (
        response.createProposalFusion.proposal &&
        response.createProposalFusion.proposal.adminUrl
      ) {
