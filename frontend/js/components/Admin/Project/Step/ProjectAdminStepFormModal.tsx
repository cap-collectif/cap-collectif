...
import type { FranceConnectAllowedData } from './ProjectAdminStepForm'
...
export const ProjectAdminStepFormModal = ({
  ...
  const [ssoConfig, setSsoConfig] = useState<boolean | null | undefined>(null)
  React.useEffect(() => {
    async function fetchData() {
      ...
      const ssoConfiguration = query?.ssoConfigurations?.edges
        ? query.ssoConfigurations.edges
            ...
            .filter(
              sso =>
                sso && sso.__typename === 'FranceConnectSSOConfiguration' && sso.isCompletelyConfigured && sso.enabled,
            )
        : []
