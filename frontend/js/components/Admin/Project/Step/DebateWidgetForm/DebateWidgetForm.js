...
export const DebateWidgetForm = ({ debateId, widget }: Props): React.Node => {
  const intl = useIntl();
  const widgetCode = getIframeCode(debateId, widget);

  return (
    <Flex direction="column">
      <Text mb={4}>{intl.formatMessage({ id: 'help-widget-integration' })}</Text>

      <Flex direction="column" spacing={2}>
        <Text>
          <Text as="span">{intl.formatMessage({ id: 'widget-appearance' })}</Text>
        ...
        <Field
          ...
          label={
            <Flex direction="row">
              <Text fontWeight={FontWeight.Normal}>
                <Text as="span">{intl.formatMessage({ id: 'destination-link' })}</Text>
                <Text as="span" color="gray.500">
                  {intl.formatMessage({ id: 'global.optional' })}
                </Text>
              </Text>
...
const DebateWidgetFormConnected = connect<any, any, _, _, _, _>(mapStateToProps)(DebateWidgetForm);
