...
export type ProposalsStepValues = Uuid | null | undefined
...
export type ProposalsProgressStateValues = 'ALL' | 'TODO' | 'IN_PROGRESS' | 'FAVOURABLE' | 'UNFAVOURABLE' | 'TOO_LATE'
export type Filters = {
  readonly state: ProposalsStateValues
  readonly progressState: ProposalsProgressStateValues
  readonly category: ProposalsCategoryValues
  readonly district: ProposalsDistrictValues
  readonly theme: ProposalsThemeValues
  readonly step: ProposalsStepValues
  readonly status: ProposalsStatusValues
...
export type ProjectAdminPageState = {
  ...
  readonly initialSelectedStep: string | null | undefined
}
export type ProjectAdminPageParameters = {
  readonly sort: $PropertyType<ProjectAdminPageState, 'sort'>
  readonly filters: $PropertyType<ProjectAdminPageState, 'filters'>
  readonly filtersOrdered: $PropertyType<ProjectAdminPageState, 'filtersOrdered'>
}
export type Action =
  | {
      type: 'START_LOADING'
  ...
  | {
      ...
      payload: ProposalsProgressStateValues
    }
  | {
...
export const getInitialState = (initialSelectedStep: string | null | undefined): ProjectAdminPageState => ({
...
export const createReducer = (state: ProjectAdminPageState, action: Action) => {
  switch (action.type) {
    ...
    case 'CHANGE_STATE_FILTER': {
      ...
      updateQueryUrl(url, 'step', {
        value: initialState.initialSelectedStep ?? '',
      })
      return { ...initialState, filters: { ...initialState.filters, state: action.payload } }
    }
    ...
    case 'CHANGE_STEP_FILTER':
      ...
      return {
        ...state,
        filters: { ...state.filters, status: 'ALL', step: action.payload },
        filtersOrdered: [
          ...(action.payload
            ? [
                {
            ...
            : []),
          ...state.filtersOrdered.filter(filter => filter.type !== 'step' && filter.type !== 'status'),
        ],
      }
    ...
    case 'CLEAR_STEP_FILTER':
      ...
      return {
        ...state,
        filters: { ...state.filters, status: 'ALL', step: null },
        filtersOrdered: [...state.filtersOrdered.filter(filter => filter.type !== 'step' && filter.type !== 'status')],
      }
    ...
    case 'CHANGE_PROGRESS_STATE_FILTER':
      updateQueryUrl(url, 'progressState', {
        value: action.payload,
      })
      return {
        ...state,
        filters: { ...state.filters, progressState: action.payload },
        filtersOrdered: [
          {
            id: action.payload,
            type: 'progressState',
