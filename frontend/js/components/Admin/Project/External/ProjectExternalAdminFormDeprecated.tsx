...
export function ProjectExternalAdminFormDeprecated(props: Props) {
  const { handleSubmit, valid, invalid, submitting, pristine, submitSucceeded, submitFailed, isExternal } = props
  return (
...
const onSubmit = (
  { externalLink, externalContributionsCount, externalParticipantsCount, externalVotesCount, isExternal }: FormValues,
  dispatch: Dispatch,
  props: Props,
) => {
  ...
  if (!externalLink || externalLink.length === 0) {
    throw new SubmissionError({
      externalLink: 'fill-field',
    })
  } else if (externalLink.toLowerCase().indexOf(hostUrl) !== -1) {
    throw new SubmissionError({
      externalLink: 'available-external-link-required',
    })
  }
