...
const ExportButton = ({ children, onChange, linkHelp, hasMarginRight = false, disabled }: Props) => {
  const intl = useIntl()
  return (
    <Container align="right" hasMarginRight={hasMarginRight} id="export-button" disabled={disabled}>
      <Collapsable.Button>
        <Icon name={ICON_NAME.download} size={12} className="no-transform-svg" color={colors.black} />
