...
import type { FormValues as ContentFormValues } from '../Content/ProjectContentAdminForm'
import ProjectContentAdminForm, { validate as validateContent } from '../Content/ProjectContentAdminForm'
import type { FormValues as ExternalFormValues } from '../External/ProjectExternalAdminForm'
import { validate as validateExternal } from '../External/ProjectExternalAdminForm'
import type { FormValues as PublishFormValues } from '../Publish/ProjectPublishAdminForm'
import ProjectPublishAdminForm, { validate as validatePublish } from '../Publish/ProjectPublishAdminForm'
import type { FormValues as AccessFormValues } from '../Access/ProjectAccessAdminForm'
import ProjectAccessAdminForm, { validate as validateProjectAccessAdmin } from '../Access/ProjectAccessAdminForm'
import type { FormValues as ProposalsFormValues } from '../Proposals/ProjectProposalsAdminForm'
import ProjectProposalsAdminForm from '../Proposals/ProjectProposalsAdminForm'
import type { ConcreteStepType } from '~relay/CreateProjectAlphaMutation.graphql'
import '~relay/CreateProjectAlphaMutation.graphql'
...
type FormValues = ContentFormValues &
  StepTypes &
  ExternalFormValues &
  PublishFormValues &
  ProposalsFormValues &
...
const onSubmit = (
  ...
  const hasNewDebateStepAdded = steps?.length > 0 ? steps.some(s => !s.id && s.__typename === 'DebateStep') : false
  const input = {
    title,
    ...
    visibility,
    address,
    restrictedViewerGroups: visibility === 'CUSTOM' ? restrictedViewerGroups.map(g => g.value) : undefined,
    opinionCanBeFollowed,
    steps: steps // I cannot type step properly given the unability to create union Input type
      ? steps.map(({ url, ...s }: any) => {
          ...
          delete s.__typename
          const requirements =
            s.requirements?.length > 0 ? s.requirements.filter(req => Object.keys(req).length > 0) : []
          return {
            ...s,
            ...
            mainView: sTypename === 'CollectStep' || sTypename === 'SelectionStep' ? s.mainView : undefined,
            requirements,
            ...
            defaultSort: sTypename === 'SelectionStep' || sTypename === 'CollectStep' ? s.defaultSort : undefined,
            votesHelpText:
              sTypename === 'SelectionStep' || sTypename === 'CollectStep'
                ? s.votable
                  ? s.votesHelpText
            ...
            publishedVoteDate:
              sTypename === 'SelectionStep' || sTypename === 'CollectStep'
                ...
                : undefined,
            secretBallot:
              sTypename === 'SelectionStep' || sTypename === 'CollectStep' ? s.isSecretBallot && s.votable : undefined,
            votesRanking: sTypename === 'SelectionStep' || sTypename === 'CollectStep' ? s.votesRanking : undefined,
            allowingProgressSteps: sTypename === 'SelectionStep' ? s.allowingProgressSteps : undefined,
            allowAuthorsToAddNews:
              sTypename === 'SelectionStep' || sTypename === 'CollectStep' ? s.allowAuthorsToAddNews : undefined,
            ...
            // DebateStep
            debateContentUsingJoditWysiwyg: sTypename === 'DebateStep' ? s.debateContentUsingJoditWysiwyg : undefined,
            articles: sTypename === 'DebateStep' ? s.articles.filter(article => article.url) : undefined,
            debate: undefined,
            slug: undefined,
  ...
  return CreateProjectAlphaMutation.commit({
    input,
  }).then(data => {
...
const renderProjectSave = ({ invalid, submitting, pristine, valid, submitSucceeded, submitFailed }: Props) => (
...
export function ProjectAdminForm(props: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} id={formName}>
      ...
      <ProjectExternalAdminPage project={project} handleSubmit={handleSubmit} formName={formName} {...rest} />
      <ProjectStepAdmin
        ...
        ? project.steps.map(step => ({
            ...
            articles:
              step.__typename === 'DebateStep'
                ...
                : [],
            hasOpinionsFilled: step.__typename === 'DebateStep' ? step?.debate?.opinions.totalCount === 2 : undefined,
            debate: {
              id: step?.debate?.id,
            },
            allowAuthorsToAddNews:
