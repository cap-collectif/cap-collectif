...
export const ImportPaperVotesFromCsvModal = ({
  ...
  const setInputAndMessages = ({ validData, errors, doubles }: AnalyseDataType): void => {
    if (validData.length === 0) {
      setError(intl.formatMessage({ id: 'count-identified-votes' }, { count: 0 }));
    } else {
  ...
  const analyseReaderResult = (result: string): void => {
    ...
    if (rows) {
      if (rows.length > 500) {
        setError(intl.formatMessage({ id: 'error-import-max-proposals' }));
      } else {
        setInputAndMessages(analyseData(rows));
      }
    } else {
      setError(intl.formatMessage({ id: 'invalid-model' }));
    }
  };
  ...
  return (
    <Modal
      hideCloseButton
      ariaLabel={intl.formatMessage({ id: 'paper-votes-add' })}
      show={show}
      width={['100%', '555px']}>
      <Modal.Header paddingY={6} borderBottom={`1px solid ${colors.borderColor}`}>
        <Flex justifyContent="space-between" alignItems="center">
          <Heading>{intl.formatMessage({ id: 'add-paper-votes' })}</Heading>
          <Icon
            name={ICON.CROSS}
            onClick={() => {
              onClose();
      ...
      <Modal.Body>
        <HelpBlock className="no-margin">
          <Text lineHeight="24px" color="gray.900" fontWeight="600">
            {intl.formatMessage({ id: 'csv-file' })}
          </Text>
          <Text lineHeight="24px" color="gray.500">
            <FormattedHTMLMessage
              id={
                isVoteRanking
                  ? 'import-paper-votes-and-points-help-text'
                  : 'import-paper-votes-help-text'
              }
              values={{
        ...
        <FileUpload
          ...
          onDrop={(files: Array<DropzoneFile>) => {
            ...
            reader.onerror = () => {
              setError(intl.formatMessage({ id: 'invalid-format' }));
            };
        ...
        {inputData.length > 0 && (
          <InfoMessage
            ...
            <InfoMessage.Title withIcon>
              {intl.formatMessage({ id: 'count-identified-votes' }, { count: inputData.length })}
            </InfoMessage.Title>
          </InfoMessage>
        )}
        {error && (
