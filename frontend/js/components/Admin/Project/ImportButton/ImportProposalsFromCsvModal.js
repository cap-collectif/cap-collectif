...
const getInputFromFile = (
  ...
  return AddProposalsFromCsvMutation.commit({
    ...
    .then((response: AddProposalsFromCsvMutationResponse) => {
      ...
      if (
        response.addProposalsFromCsv &&
        response.addProposalsFromCsv.mandatoryMissing.length >= 1
      ) {
    ...
    .catch(() => {
      ...
      output.verificationFailed = () => (
        <Flex fontSize="11px" fontWeight="400">
          {intl.formatMessage({ id: 'verification-failed' })}&nbsp;
          <Button
            fontWeight="600"
            lineHeight="0"
            fontSize="11px"
            ...
            <u>{intl.formatMessage({ id: 'button.try.again' })}</u>
          </Button>
        </Flex>
      );
      dispatch(
...
export default connect<any, any, _, _, _, _>(mapStateToProps)(formContainer);
