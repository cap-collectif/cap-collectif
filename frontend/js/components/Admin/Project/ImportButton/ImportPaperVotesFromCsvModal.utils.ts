...
const getCsvHeaders = (isVoteRanking: boolean): Array<string> => {
  return isVoteRanking ? ['reference', 'title', 'votes-total', 'points-total'] : ['reference', 'title', 'votes-total']
}

const getProposalIdFromFullReference = (fullReference: string, proposals: Array<ProposalType>): string => {
  ...
  return encodeURI(
    `data:text/csv;charset=utf-8,${universalBomForExcel}${getCsvHeaders(isVoteRanking).join(';')}\r\n${proposals
      .map(proposal => {
        ...
        if (isVoteRanking) {
          const points = proposal.paperVotesTotalPointsCount > 0 ? proposal.paperVotesTotalPointsCount : ''
...
export const isAlreadyThere = (data: Array<UpdatePaperVoteInput>, newSplitRow: Array<string>): boolean => {
  return data.some(datum => datum.proposal === newSplitRow[REFERENCE].replace(/"/g, ''))
}
export const splitResult = (result: string): Array<string> | null | undefined => {
