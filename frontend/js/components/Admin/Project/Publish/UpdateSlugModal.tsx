...
`
type Props = {
  readonly project: UpdateSlugModal_project$key | null | undefined
...
const onSubmit = (formValues: FormValues, dispatch: Dispatch, props: Props) => {
  ...
  return UpdateProjectSlugMutation.commit({
    input,
  }).then(response => {
    onClose()
        ...
        <Modal.Body>
          ...
          <Flex mb={2} mr={2}>
            <Field type="checkbox" id="agree" name="agree" component={renderComponent}>
              <Label htmlFor="agree">
                {intl.formatMessage({
                  id: 'understood-cannot-cancel',
                })}
              </Label>
            </Field>
          </Flex>
        </Modal.Body>
        <Modal.Footer justifyContent="space-between" borderTop="normal" borderColor="gray.200" pt={6}>
          <Flex align="center">
            <Button
          ...
          <ButtonGroup>
            <Button variant="secondary" variantSize="big" variantColor="hierarchy" onClick={() => onClose()}>
              {intl.formatMessage({
                id: 'global.cancel',
              })}
            </Button>
