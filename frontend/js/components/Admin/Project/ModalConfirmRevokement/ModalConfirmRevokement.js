...
const AnalystRevokedList: StyledComponent<{}, {}, HTMLUListElement> = styled.ul`
  display: flex;
...
const ModalConfirmRevokement = ({
  ...
  React.useEffect(() => {
    const analystsRemovedWithAnalyseBegin = ((analystsRevoked
      ...
      .map(analystId =>
        analystsWithAnalyseBegin.find(({ id }) => id === analystId),
      ): any): AnalystsDisplay);
    ...
    <Modal
      ...
      <Modal.Body>
        <FormattedMessage
          ...
          values={{ num: analystsDisplay.length }}
        />
