...
type ReduxProps = {
  readonly proposalRevisionsEnabled: boolean
  readonly viewerIsAdmin: boolean
}
type Props = ReduxProps & {
  readonly query: ProjectAdminProposalsPage_query
...
type VariableQuery = {
  ...
  readonly orderBy: {
    readonly field: 'VOTES' | 'POINTS' | 'PUBLISHED_AT' | 'NUMBER_OF_MESSAGES_RECEIVED'
...
const ProjectAdminProposalsPage = ({
  ...
  const hasFilters: boolean = !isEqual(
    { ...initialVariables(viewerIsAdmin), projectId, step: firstCollectStepId, proposalRevisionsEnabled },
  ...
  return (
    <Skeleton
      ...
      placeholder={
        <ProjectAdminProposalsPlaceholder hasError={!!error} fetchData={retry} selectedTab={parameters.filters.state} />
      }
    >
