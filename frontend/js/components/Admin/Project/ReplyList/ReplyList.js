...
type Props = {|
  +questionnaire: ReplyList_questionnaire$key,
  +term: string,
  +resetTerm: () => void,
|};
...
const ReplyList = ({ questionnaire, term, resetTerm }: Props): React.Node => {
  const intl = useIntl();
  ...
  return (
    <Table
        ...
        <ReplyModalConfirmationDelete
          ...
          disclosure={
            <Button variantSize="small" variant="secondary" variantColor="danger">
              {`${intl.formatMessage({ id: 'global.remove' })} (${selectedRows.length})`}
            </Button>
          }
        />
      )}>
      <Table.Thead>
        <Table.Tr>
          <Table.Th>
            {intl.formatMessage({ id: 'count-answers' }, { num: adminReplies.totalCount })}
          </Table.Th>
          <Table.Th>{intl.formatMessage({ id: 'global.author' })}</Table.Th>
          <Table.Th>
            {({ styles }) => (
              <Menu>
                ...
                <Menu.List>
                  <Menu.OptionGroup
                    ...
                    <Menu.OptionItem value="PUBLISHED">
                      <Text>{intl.formatMessage({ id: 'global.published' })}</Text>
                    </Menu.OptionItem>
                    <Menu.OptionItem value="NOT_PUBLISHED">
                      <Text>{intl.formatMessage({ id: 'global.no.published' })}</Text>
                    </Menu.OptionItem>
                    <Menu.OptionItem value="DRAFT">
                      <Text>{intl.formatMessage({ id: 'global-draft' })}</Text>
                    </Menu.OptionItem>
              ...
              <Menu>
                ...
                <Menu.List>
                  <Menu.OptionGroup
                    ...
                    onChange={direction => setOrderBy({ field: 'UPDATED_AT', direction })}
                    type="radio"
                    title={intl.formatMessage({ id: 'sort-by' })}>
              ...
              <Menu>
                ...
                <Menu.List>
                  <Menu.OptionGroup
                    ...
                    onChange={direction => setOrderBy({ field: 'CREATED_AT', direction })}
                    type="radio"
                    title={intl.formatMessage({ id: 'sort-by' })}>
                    <Menu.OptionItem value="DESC">
                      <Text>{intl.formatMessage({ id: 'global.filter_last' })}</Text>
