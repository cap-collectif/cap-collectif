...
const ProjectAdminProposalsPlaceholder = ({ hasError, fetchData, selectedTab }: Props) => {
  ...
  return (
    <>
      <Header>
        <div>
          <Tab selected={selectedTab === 'ALL'}>
            {intl.formatMessage({
              id: 'fast.filter.skeleton.all',
            })}
          </Tab>
          <Tab selected={selectedTab === 'PUBLISHED'}>
            {intl.formatMessage({
              id: 'fast.filter.skeleton.published',
            })}
          </Tab>
          <Tab selected={selectedTab === 'DRAFT'}>
            {intl.formatMessage({
              id: 'fast.filter.skeleton.draft',
            })}
          </Tab>
          <Tab selected={selectedTab === 'TRASHED'}>
            {intl.formatMessage({
              id: 'fast.filter.skeleton.trash',
            })}
          </Tab>
          <Tab selected={selectedTab === 'ARCHIVED'}>
            {intl.formatMessage({
              id: 'fast.filter.skeleton.archived',
            })}
          </Tab>
        </div>
        <div>
      ...
      <AnalysisPlaceholderDashboard type={TYPE_DASHBOARD.BO_CONTRIBUTION} hasError={hasError} fetchData={fetchData} />
    </>
  )
}
