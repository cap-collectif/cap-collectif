...
export const MediaAdminList = ({ query, relay }: Props) => {
  ...
  const [viewedMedia, setViewedMedia] = useState<string | null | undefined>(null)
  const [deletedMedia, setDeletedMedia] = useState<string | null | undefined>(null)
  ...
  return (
    ...
    >
      <MediaViewModal show={showMediaViewModal} closeModal={() => setShowMediaViewModal(false)} url={viewedMedia} />
      <DeleteModal
        closeDeleteModal={() => {
          ...
          <S.MediaHeaderList>
            <S.MediaColumn size={400}>
              <FormattedMessage
                ...
                values={{
                  num: selectedRows.length || medias.totalCount,
                }}
              />
              {selectedRows.length > 0 && (
                <button type="button" className="delete ml-5" onClick={() => setShowDeleteModal(true)}>
                  <FormattedMessage id="global.delete">{(msg: string) => <>{msg}</>}</FormattedMessage>
                </button>
              )}
            </S.MediaColumn>
            ...
            {medias.edges
              ...
              .map(media => (
                <S.MediaRow rowId={media.id} key={media.id}>
                    ...
                    <div>
                      {isImage(media.providerReference) && (
                        <>
                          ...
                          >
                            <FormattedMessage id="action_show">{(msg: string) => <>{msg}</>}</FormattedMessage>
                          </button>
                          <span>|</span>
                        </>
                      )}
                  ...
                  <S.MediaColumn size={100}>{getFileExtension(media.providerReference)}</S.MediaColumn>
                  <S.MediaColumn size={100}>
