...
const asyncValidate = (values: { inputEmails: string; redirectionUrl: string }) => {
  if (values.inputEmails !== '') {
    ...
    if (formattedWrongInputEmails.length > 0) {
      return new Promise((resolve, reject) => {
        reject({
          inputEmails: 'input-emails-wrong-format',
        })
      })
    }
    ...
    if (emails.length > maxEmails) {
      return new Promise((resolve, reject) => {
        reject({
          inputEmails: 'input-emails-max-reached',
        })
      })
    }
        ...
        fetchQuery(environment as any, USER_FETCH_QUERY, {
          ...
          next: response => {
            ...
            if (invitationsAvailabilitiesData.totalCount > 0) {
              const duplicateEmails = invitationsAvailabilitiesData.edges.filter(item => !item.node.availableForUser)

              if (duplicateEmails.length > 0) {
                if (emails.length > 1 && duplicateEmails.length === emails.length) {
                  reject({
                    inputEmails: 'input-email-already-used',
                  })
                }
  ...
  if (values.redirectionUrl !== '') {
    ...
    if (!hostname.test(values.redirectionUrl)) {
      return new Promise((resolve, reject) => {
        reject({
          redirectionUrl: 'input-redirection-url-match-error',
        })
      })
    }
  }
...
`
type BeforeProps = {
  readonly query: UserInviteModalSteps_query$key
  readonly intl: IntlShape
}
type Props = ReduxFormFormProps & BeforeProps
export const UserInviteModalSteps = ({ query: queryFragment, intl, ...props }: Props): JSX.Element => {
  const dispatch = useDispatch<Dispatch>()
  const query = useFragment(FRAGMENT, queryFragment)
  const csvEmails = useSelector((state: GlobalState) => formValueSelector('form-user-invitation')(state, 'csvEmails'))
        ...
        <>
          ...
          <ModalSteps.Body>
            ...
            <UserInviteModalStepsChooseRole
              ...
              id="choose-role"
              label={intl.formatMessage({
                id: 'user-invite-settings',
              })}
              validationLabel={intl.formatMessage({
                id: 'confirm-sending-invitation',
              })}
            />
...
})(UserInviteModalSteps) as React.AbstractComponent<BeforeProps>
