...
export const UserInviteRelaunchInvitations = ({ invitations }: Props): JSX.Element => {
  ...
  const relaunchedInvitations = relaunchableInvitations.filter(invitation => invitation.relaunchCount > 0)
  const notRelaunchedInvitations = relaunchableInvitations.filter(invitation => invitation.relaunchCount === 0)
  const invitationsToRelaunched = exclude ? notRelaunchedInvitations : relaunchableInvitations
  return (
    <Modal
      ariaLabel={intl.formatMessage({
        id: 'invitations.confirm.relaunch',
      })}
      disclosure={
        <Button
          variantSize="small"
