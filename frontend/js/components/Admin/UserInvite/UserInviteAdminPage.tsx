...
import '~relay/UserInviteAdminPageQuery.graphql'
...
export const UserInviteAdminPageQuery: GraphQLTaggedNode = graphql`
  query UserInviteAdminPageQuery($first: Int, $cursor: String, $term: String, $status: UserInviteStatus) {
...
const UserInviteAdminPage = ({ queryReference }: Props): JSX.Element => {
  const intl = useIntl()
  const query = usePreloadedQuery<UserInviteAdminPageQueryType>(UserInviteAdminPageQuery, queryReference)
  ...
  return (
    <Flex direction="column" spacing={3}>
      <Flex direction="row" justify="space-between" align="center" px={6} py={4} bg="white">
        ...
        <Flex direction="row" color="blue.800" spacing={4} {...headingStyles.h5} fontWeight={FontWeight.Semibold}>
          <Text>
            ...
            {intl.formatMessage(
              {
                id: 'invitations-count',
              },
              {
                num: allInvitations.totalCount,
              },
            )}
          </Text>
