...
describe('<CarouselDesktop />', () => {
  ...
  it('should render correctly', () => {
    const wrapper = shallow(<CarouselMobile {...props} />);
    expect(wrapper).toMatchSnapshot();
  });

  it('should render correctly negative translation', () => {
    const wrapper = shallow(<CarouselMobile {...propsEmptyItem} />);
    wrapper.setState({ translateX: -300 });
    const mobileContent = wrapper.find('#mobile-content').prop('style');
    expect(mobileContent).toHaveProperty('transform', 'translateX(-300px)');
  });
});
