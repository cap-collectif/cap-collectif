...
import '../ProposalFormAdminCategoriesStepModal'
...
type Props = {
  ...
  readonly customCategoryImage: CategoryImage | null | undefined
}
...
const CircledIcon = styled.div`
  ...
  padding-top: 1px;
`
export const ProposalFormCategoryBackgroundPreview = ({ color, name, icon, customCategoryImage }: Props) => {
  const { width } = useResize()
  return (
    <Container hasTitle={!!name}>
      {customCategoryImage ? (
        <Image src={customCategoryImage?.url || customCategoryImage.image?.url} alt="Category background" />
      ) : (
        <>
