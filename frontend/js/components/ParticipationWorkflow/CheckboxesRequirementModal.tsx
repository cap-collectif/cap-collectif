...
const CheckboxesRequirementModal: React.FC<Props> = ({ checkboxes, hideGoBackArrow }) => {
  ...
  const isLoading = updateParticipantRequirementMutation.isLoading || updateRequirementMutation.isLoading
      ...
      >
        <Box as="form" width="100%" onSubmit={handleSubmit(onSubmit)}>
          <Text mb={0} fontWeight={400}>
            {intl.formatMessage({ id: 'check-the-checkboxes' }, { num: checkboxes.length })}
          </Text>
          {checkboxes.map(requirement => {
            return (
              <FormControl mb={0} name={`checkboxes.${requirement.id}`} control={control} key={requirement.id}>
                <FieldInput
          ...
          <Button
            ...
            isLoading={isLoading}
          >
            {intl.formatMessage({ id: 'check-everything-and-continue' }, { num: checkboxes.length })}
          </Button>
        </Box>
      </ModalLayout>
    </>
