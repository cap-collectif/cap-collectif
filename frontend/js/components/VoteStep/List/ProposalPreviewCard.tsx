...
export const ImageContainer = ({ children }: { children: JSX.Element | JSX.Element[] | string }) => (
...
const IconAndText = ({
  ...
  return (
    <Flex alignItems="center" spacing={1} width={width} justifyContent={justifyContent} mr={mr}>
      ...
      {children ? (
        <Text as="span" fontSize={2} color={color || 'gray.500'} truncate={truncate ? 80 : undefined}>
          {children}
        </Text>
      ) : null}
    </Flex>
...
export const ProposalPreviewCard = ({
  ...
  readonly proposal: ProposalPreviewCard_proposal$key | null | undefined
  ...
  const { media, author, category, title, district, summary, body, votes, status, theme } = proposal
  ...
  return (
    ...
    >
      ...
      >
        <Flex direction="column" width={['100%', '100%', '100%', showImage ? 'calc(50% - 4rem)' : '100%']}>
          <Flex mb={2} justify={['space-between', 'start']} flexWrap="wrap">
            ...
            {theme ? (
              <IconAndText truncate useOldIcons icon={null} size={isMobile ? 16 : 13} mr={isMobile ? 0 : 6}>
                {isMobile ? null : theme.title}
              </IconAndText>
            ) : null}
          </Flex>
          <Link href={`${projectSlug || ''}/${url}/${proposal.slug}`} stepId={stepId}>
            <Heading as="h3" fontSize={4} color={['gray.900', '#CE237F']} mb={2} lineHeight="initial">
              {title}
            </Heading>
            <Text
              as="div"
          ...
          <Flex
            ...
            mt={[4, 6]}
          >
            <Flex alignItems="center" spacing={4} width={['100%', '']} justifyContent={['center', 'start']} mt={[6, 0]}>
              <VoteButton hasVoted={hasVoted} stepId={stepId} proposalId={proposal.id} disabled={disabled} />
              <Flex
                alignItems="center"
                spacing={2}
                display={['none', showImage ? 'flex' : 'none', showImage ? 'flex' : 'none', 'flex']}
              >
        ...
        {showImage ? (
          <ImageContainer>
            <Link href={`${projectSlug || ''}/${url}/${proposal.slug}`} stepId={stepId}>
              {media ? (
                <Image src={media?.url} alt={media?.name} height="100%" width="100%" objectFit="cover" />
              ) : (
                <>
                  {category?.icon ? (
                  ...
                  <CategoryBackground color={category?.color || ACTIVE_COLOR} viewBox="5 0 240 80" height="100%" />
                </>
              )}
            </Link>
          </ImageContainer>
