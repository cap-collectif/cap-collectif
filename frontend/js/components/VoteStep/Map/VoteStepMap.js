...
type Props = {|
  ...
  +DEFAULT_CENTER: { ...MapCenterObject, zoom: number },
  +stepId: string,
  +disabled: boolean,
|};
...
export const VoteStepMap = ({
  ...
  const proposal = data.proposals.edges
    ?.map(edge => edge?.node)
  ...
  const debouncedOnPositionChange = debounce(
    (map: MapProps) => onPositionChange(map),
    DELAY_BEFORE_MAP_RELOADING,
  );
      ...
      <MapContainer>
        <Map
          whenCreated={(map: MapProps) => {
            mapRef.current = map;
            if (!isMobile) {
              if (filterBounds && mapRef.current)
                mapRef.current.fitBounds(boundsToLeaflet(filterBounds));
          ...
          center={urlCenter || { lat: DEFAULT_CENTER.lat, lng: DEFAULT_CENTER.lng }}
                  ...
                  <Marker
                    ...
                    eventHandlers={{
                      click: () => {
                        ...
                        else dispatchEvent(VoteStepEvent.ClickProposal, { id: mark.id });
                      },
                    }}
                  />
                );
        ...
        <AnimatePresence initial={false} onExitComplete={() => setSelectedProposal(null)}>
          {selectedProposal && proposal ? (
            <motion.div
              ...
              exit={{ opacity: 0, y: 321 }}
              transition={{ delay: DELAY_BEFORE_PROPOSAL_REMOVAL * 3 }}>
