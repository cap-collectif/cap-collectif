...
const VoteButton = ({ proposalId, stepId, hasVoted, disabled }: Props) => {
  ...
  const deleteVote = () => {
    ...
    return RemoveProposalVoteMutation.commit({
      stepId,
      input: { proposalId, stepId },
      isAuthenticated,
      token: null,
    })
      .then(() => {
        dispatchEvent(VoteStepEvent.RemoveVote, { proposalId });
        setHasSwitch(true);
        setIsLoading(false);
      })
      .catch(() => {
  ...
  const onButtonClick = () => {
    dispatchEvent(VoteStepEvent.AnimateCard, { proposalId });
    if (hasVoted) deleteVote();
      ...
      vote(
        ...
        intl,
        () => {
          dispatchEvent(VoteStepEvent.AddVote, { proposalId });
          setHasSwitch(true);
          setIsLoading(false);
        },
        () => setIsLoading(false),
      ...
      <LoginOverlay enabled={!isAuthenticated}>
        <Box
          ...
          <Flex className="toggle-btn" alignItems="center">
            <Text
              as="span"
              color={displayhasVoted ? 'white' : ACTIVE_COLOR}
              fontSize={3}
              fontWeight="700"
              mx={2}>
              {intl.formatMessage({ id: leftText })}
            </Text>
            <Flex
              className="inner-circle"
              p={1}
              borderRadius="100%"
              bg={displayhasVoted ? 'white' : ACTIVE_COLOR}>
              <Icon
                name={CapUIIcon.Vote}
                size={CapUIIconSize.Lg}
                color={displayhasVoted ? ACTIVE_COLOR : 'white'}
              />
            </Flex>
          </Flex>
        </Box>
