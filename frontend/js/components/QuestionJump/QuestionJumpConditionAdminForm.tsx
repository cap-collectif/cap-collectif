...
export class QuestionJumpConditionAdminForm extends React.Component<Props> {
  handleQuestionChange = (e: React.SyntheticEvent<HTMLSelectElement>) => {
  ...
  displayValueList = (questions: QuestionsInReduxForm, selectedQuestion: string) => {
    const question: any = questions.find(q => q.id === selectedQuestion)

    if (question) {
      return question.choices // @ts-expect-error we need to refacto the responseHelper.js file and its 1131 lines
        ? question?.choices
  ...
  render() {
    const { intl, index, questions, selectedQuestion, member, dispatch, formName, oldMember } = this.props
    return (
      <div className="movable-element" key={index}>
        ...
        <Field id={`${member}.operator`} name={`${member}.operator`} type="select" component={component}>
          <option value="IS">
            {intl.formatMessage({
              id: 'is',
            })}
        ...
        <Field id={`${member}.value.id`} name={`${member}.value.id`} type="select" component={component}>
