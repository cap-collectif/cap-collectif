    ...
    return (
      <div id={`opinion-type--${slug}`} className="anchor-offset text-center">
        ...
        {!hideEmptySection && (contributionsCount > 0 || contribuable) && (
          <div className="mt-15">
            <OpinionList enablePagination={enablePagination} consultation={consultation} section={section} />
          </div>
        )}
      </div>
    )
...
export default createFragmentContainer(Section, {
  ...
  consultation: graphql`
    fragment Section_consultation on Consultation @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
