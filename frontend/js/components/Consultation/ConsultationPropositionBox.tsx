...
export const ConsultationPropositionBox = (props: Props) => {
  ...
  const scrollSpy = () => {
    ...
    sectionItems.forEach(item => {
      ...
      if (itemPosition && itemPosition.top - 20 < 0 && itemPosition.top - 20 > -itemPosition.height + navHeight) {
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('scroll', animationFrame, false) // eslint-disable-next-line
  }, [])
...
const mapStateToProps = (state: GlobalState, props: Props) => ({
  showConsultationPlan:
    props.id in state.project.showConsultationPlanById ? state.project.showConsultationPlanById[props.id] : true,
