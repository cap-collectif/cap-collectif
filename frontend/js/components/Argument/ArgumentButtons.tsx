...
const ArgumentButtons = ({ argument, dispatch }: Props) => {
  ...
  return (
    <div className="small">
      ...
      <ShareButtonDropdown id={`arg-${argument.id}-share-button`} url={argument.url} bsSize="xs" outline grey />
    </div>
  )
}

// @ts-ignore
const container = connect<any, any>()(ArgumentButtons)
export default createFragmentContainer(container, {
  argument: graphql`
    fragment ArgumentButtons_argument on Argument @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
      author {
        id
        slug
        displayName
