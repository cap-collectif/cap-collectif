...
type Props = {
  readonly debateArgument: DebateArgumentItem_debateArgument
    ...
    <AppBox as="li" p={6} borderRadius={6} border="card" borderColor="gray.200" mb={6} bg="white">
      ...
      <Flex flexDirection="row" spacing={2} alignItems="center">
        ...
        <Tag variant={debateArgument.type === 'FOR' ? 'green' : 'red'} interactive={false}>
          <FormattedMessage
            id={debateArgument.type === 'FOR' ? 'argument.show.type.for' : 'argument.show.type.against'}
          />
        </Tag>
        <Text
          fontSize={14}
        ...
        >
          <FormattedMessage
            ...
            values={{
              date: (
                <FormattedDate value={moment(debateArgument.publishedAt)} day="numeric" month="short" year="numeric" />
              ),
              time: <FormattedDate value={moment(debateArgument.publishedAt)} hour="numeric" minute="numeric" />,
            }}
          />
        </Text>
      </Flex>
      {isEditing ? (
        <ArgumentCardFormEdition argument={debateArgument} goBack={() => setIsEditing(false)} intl={intl} />
      ) : (
        <Text>
          {readMore
            ? debateArgument.body
            : truncate(debateArgument.body, {
