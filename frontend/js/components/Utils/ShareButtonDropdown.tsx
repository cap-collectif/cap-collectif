...
const ShareButtonDropdown = ({ id = 'share-button', title = '', url, enabled, bsSize, disabled }: Props) => {
  ...
  const openSharer = (href: string, name: string) => {
    ...
    window.open(href, name, `top=${top},left=${left},menubar=0,toolbar=0,status=0,width=${width},height=${height}`)
  }
  ...
  const linkedin = () => {
    openSharer(`https://www.linkedin.com/shareArticle?mini=true&url=${getEncodedUrl()}&text=${title}`, 'Linkedin')
  }

  const renderModal = () => {
    return (
      <Modal show={isOpen} onHide={onClose} animation={false} dialogClassName="modal--custom modal--share-link">
        <Modal.Header
          closeButton
    ...
    <Menu
      disclosure={
        <Button
          ...
          leftIcon={<Icon name={CapUIIcon.Share} size={bsSize === 'xs' ? CapUIIconSize.Xs : CapUIIconSize.Sm} />}
        >
          {intl.formatMessage({
            id: 'global.share',
          })}
...
export default connect<any, any>(mapStateToProps)(ShareButtonDropdown)
