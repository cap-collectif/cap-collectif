...
`
type Props = {
  readonly pendingInvitationFragmentRef: GroupAdminPendingInvitationsList_group$key
}
...
const GroupAdminPendingInvitationsList = ({ pendingInvitationFragmentRef }: Props) => {
  ...
  const {
    ...
    loadNext,
    isLoadingNext,
  } = usePaginationFragment(FRAGMENT, pendingInvitationFragmentRef)
  if (group.pendingInvitations?.edges && group.pendingInvitations.edges.length <= 0) return null
  return (
    <>
