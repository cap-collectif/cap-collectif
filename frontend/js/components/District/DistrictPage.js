...
const QUERY = graphql`
  query DistrictPageQuery($count: Int!, $cursor: String, $districtId: ID!) {
    ...DistrictPageProjectList_query
      @arguments(count: $count, cursor: $cursor, districtId: $districtId)
...
export const DistrictPageLayout = ({
  queryReference,
}: {
  queryReference: PreloadedQuery<DistrictPageQueryType>,
}) => {
  ...
  return (
    <>
      ...
      <Flex maxWidth="1200px" margin="auto" mb={6} px={[3]} justify="space-between">
        <Flex direction="column" maxWidth={['100%', '60%']}>
          <Text as="div" maxWidth={['100%', '90%']} mb={[8, 0]}>
