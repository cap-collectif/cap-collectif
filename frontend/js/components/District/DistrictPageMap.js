...
const DistrictPageMap = ({ geojson }: Props) => {
  ...
  const geoJSON = formatGeoJsons([{ geojson, displayedOnMap: true, id: '' }], true);
        ...
        whenCreated={map => {
          ...
          if (hull.geometry.type === 'MultiPolygon') {
            ...
            for (let i = 0; i < (hull.geometry.coordinates[0].length || 0); i++) {
              latLngs.push(
                new L.LatLng(
                  hull.geometry.coordinates[0][i][1],
                  hull.geometry.coordinates[0][i][0],
                ),
              );
            }
          }
