...
import { FormattedDate, FormattedNumber, FormattedMessage, FormattedTime, injectIntl } from 'react-intl'
      ...
      <Col md={colTitleWidth()} sm={12} xs={12}>
        <div>
          <div>
            ...
            {vote.createdAt ? (
              <FormattedMessage
                id={isInterpellationContextFromStep(step) ? 'supported.date-at-time' : 'voted-on-date-at-time'}
                values={{
                  date: <FormattedDate value={moment(vote.createdAt)} day="numeric" month="long" year="numeric" />,
                  time: <FormattedTime value={moment(vote.createdAt)} hour="numeric" minute="numeric" />,
                }}
              />
            ) : (
              <FormattedMessage
