...
const FRAGMENT = graphql`
    ...
    @refetchable(queryName: "RestrictedViewersLegacyListQuery") {
        ...
        @connection(key: "UserGroupModalLegacy_restrictedViewers") {
            edges {
                node {
                    id
                    title
...
const UserGroupModalLegacy = ({ show, project, handleClose }: Props): React.Node => {
  const intl = useIntl();
  ...
  return (
    <Modal
      ...
      <Modal.Body>
        ...
        data.restrictedViewers.edges.length > 0 && (
            ...
            {hasNext && (
              <div className="text-center">
                <Button variant="primary" onClick={loadMore} disabled={isLoadingNext}>
                  {intl.formatMessage({ id: isLoadingNext ? 'global.loading' : 'global.more' })}
                </Button>
              </div>
            )}
          </ListGroupFlush>
