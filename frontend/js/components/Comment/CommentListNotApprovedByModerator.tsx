...
import '~relay/CommentListNotApprovedByModerator_commentable.graphql'
type Props = {
  readonly commentable: CommentListNotApprovedByModerator_commentable$key
...
const CommentListNotApprovedByModerator = ({ commentable: commentRef, useBodyColor }: Props) => {
  ...
  const nodes = commentable?.viewerNotApprovedByModeratorComments?.edges?.filter(Boolean)?.map(edge => edge.node) ?? []
