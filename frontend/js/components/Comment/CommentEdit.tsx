...
export const CommentEdit = ({ comment }: Props) => {
  ...
  const canEdit = (comment.contribuable && comment?.author?.isViewer && !isApproved) || comment?.author?.isAdmin
...
export default createFragmentContainer(CommentEdit, {
  comment: graphql`
    fragment CommentEdit_comment on Comment @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
      id
      contribuable
      moderationStatus
      author {
