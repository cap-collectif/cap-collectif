...
type Props = ReduxFormFormProps & {
  ...
  readonly isAnonymousQuestionnaireFeatureEnabled: boolean
...
const onSubmit = (values: FormValues, dispatch: Dispatch, props: Props, state: GlobalState) => {
        ...
        .then(() => {
          toast({
            ...
            content: intl.formatHTMLMessage({
              id: values.draft ? 'your-answer-has-been-saved-as-a-draft' : 'reply.request.create.success',
            }),
          })
          history.replace('/')
        })
  ...
  if (!isAuthenticated) {
    return AddAnonymousReplyMutation.commit({
      ...
      .then(response => {
        ...
        if (anonymousReply?.token && anonymousReply?.reply?.id && anonymousReply.questionnaire?.id) {
  ...
  return AddUserReplyMutation.commit({
    ...
    .then(response => {
      if (response.addUserReply?.errorCode && response.addUserReply?.errorCode === 'REQUIREMENTS_NOT_MET') {
        toast({
          variant: 'danger',
          content: intl.formatMessage({
            id: 'add_reply_requirements_not_met',
          }),
        })
...
export class ReplyForm extends React.Component<Props, State> {
  ...
  formIsDisabled() {
    ...
    if (!canParticipateAnonymously) {
      return (
        ...
        (!questionnaire?.viewerReplies?.totalCount === 0 && !questionnaire.multipleRepliesAllowed && !reply)
      )
    }
  ...
  render() {
    ...
    const disabledSubmitBtn = () => {
      ...
      const isDisabled =
        invalidRequirements || invalid || submitting || disabled || (!isDraft && pristine) || !isNativeFormValid
    ...
    return (
      <QuestionnaireContainer id="reply-form" onSubmit={handleSubmit} ref={this.formRef}>
        ...
        <ReplyFormContainer id="reply-form-container">
          {(newReplyOrReplyInDraft || replyIsPublished) && stepHasRequirement && user && !hasFilledRequirements && (
            <>
          ...
          {!isAuthenticated && questionnaire.step?.collectParticipantsEmail && canParticipateAnonymously && (
            <>
...
const mapStateToProps = (state: GlobalState, props: Props) => {
  ...
  return {
    responses:
      formValueSelector(reply ? getFormNameUpdate(reply.id) : `Create${formName}`)(state, 'responses') ||
      defaultResponses,
    initialValues: {
