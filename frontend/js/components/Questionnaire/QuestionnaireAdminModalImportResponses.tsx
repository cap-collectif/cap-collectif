...
type Props = {
  ...
  type: string
  oldMember: string
  asyncValidating: string | null | undefined
}
type DefaultProps = void
type State = {
  analyzed: boolean
  files: Array<DropzoneFile> | null | undefined
}
export class QuestionnaireAdminModalImportResponses extends React.Component<Props, State> {
  ...
  onPostDrop = (droppedFiles: Array<DropzoneFile>, input: Record<string, any>, oldMember: string, type: string) => {
    this.setState({
      analyzed: true,
      files: droppedFiles,
    })
      ...
      <Modal show={show} onHide={onClose} aria-labelledby="delete-modal-title-lg">
        ...
        <Modal.Footer>
          <CloseButton
            ...
            onClose={onClose}
          />
          <Button disabled={pristine || submitting} bsStyle="primary" type="button" id="import-file" onClick={onAdd}>
