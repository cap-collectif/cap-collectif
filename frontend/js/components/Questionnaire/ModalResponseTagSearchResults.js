...
type Props = {|
  ...
  +show: boolean,
  +onClose: () => void,
  +searchTag: {| value: string, count: number |},
|};

const QUERY = graphql`
  query ModalResponseTagSearchResultsQuery(
    $questionId: ID!
    $term: String!
    $first: Int!
    $cursor: String
  ...
  return (
    <Modal
      ...
      <Heading
        ...
        <FormattedMessage id="tag-cloud-answer-number" values={{ count: searchTag.count }} />
        <Text ml={2} fontWeight="600">{` "${searchTag.value}"`}</Text>
      </Heading>
      <Modal.Body style={{ padding: '24px', overflow: 'scroll', maxHeight: '80vh' }}>
        {!props ? (
          <>
            <Loader />
            <Loader />
