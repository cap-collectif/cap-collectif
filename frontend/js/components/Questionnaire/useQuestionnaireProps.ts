...
const generateTranslations = (
  ...
  if (!isTimeless) {
    ...
    const startAt = step?.timeRange?.startAt ? moment(step?.timeRange?.startAt).format('DD/MM/YY') : ''
    const endAt =
  ...
  return {
    attendee: intl.formatMessage(
      ...
      {
        num: participants.totalCount,
      },
    ),
    particpationAllowed:
      participationAllowedKey === ''
        ? ''
        : intl.formatMessage({
            id: participationAllowedKey,
          }),
    resultsCollectedBetweenDates: resultsCollectedBetweenDatesMessage,
    keyword: intl.formatMessage({
      id: 'global.keyword',
    }),
    occurence: intl.formatMessage({
      id: 'global.occurence',
    }),
    tableContent: intl.formatMessage({
      id: 'table-content',
    }),
    optional: intl.formatMessage({
      id: 'global.optional',
    }),
    noReply: intl.formatMessage({
      id: 'global.no-reply',
    }),
  }
}
...
export const useQuestionnaireProps = (
  ...
  const intl = useIntl()
  const [translations, setTranslations] = useState<Translations | null | undefined>(null)
  const [questions, setQuestions] = useState<QuestionsType | null | undefined>(null)
  useEffect(() => {
