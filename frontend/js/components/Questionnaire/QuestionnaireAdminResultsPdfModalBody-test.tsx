...
describe('<QuestionnaireAdminResultsPdfModalBody />', () => {
  ...
  const errorProps = { ...props, loading: false, error: true }
  const retryCountExceedLimitProps = { ...errorProps, retryCount: 1 }
  ...
  it('renders correctly when retryCount exceed limit', () => {
    const wrapper = shallow(<QuestionnaireAdminResultsPdfModalBody {...retryCountExceedLimitProps} />)
    expect(wrapper).toMatchSnapshot()
  })
})
