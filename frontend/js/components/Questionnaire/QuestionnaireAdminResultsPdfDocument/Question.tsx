...
type Props = {
  question: QuestionPdfType | null | undefined
  ...
  return (
    ...
    >
      <Text style={styles.question_title}>
        ...
        {question?.required === false && <Text style={styles.facultative}>{translations?.optional}</Text>}
      </Text>
      ...
      {question?.__typename === 'SimpleQuestion' && question?.tagCloud && question?.tagCloud?.length > 0 && (
        ...
        >
          <TagsTable
            tags={question?.tagCloud?.slice(0, Math.ceil(question?.tagCloud.length / 2))}
            translations={translations}
          />
          <TagsTable
            tags={question?.tagCloud?.slice(Math.ceil(question?.tagCloud.length / 2))}
            translations={translations}
          />
        </View>
      )}
