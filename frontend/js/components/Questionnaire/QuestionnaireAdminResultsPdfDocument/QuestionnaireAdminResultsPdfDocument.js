...
export type QuestionPdfType = QuestionType & {
  +category: 'question' | 'section' | 'subsection',
  +position?: number,
  +sectionIndex?: number,
  +subSectionIndex?: number,
};

export type QuestionsPdfType = ?$ReadOnlyArray<QuestionPdfType>;

const QuestionnaireAdminResultsPdfDocument = ({
  title,
  step,
  questions,
  logoUrl,
            ...
            if (question?.category === 'subsection') {
              return (
                <Text key={index} style={styles.question_subsection}>
                  {convertToRoman(question?.sectionIndex)} -{' '}
