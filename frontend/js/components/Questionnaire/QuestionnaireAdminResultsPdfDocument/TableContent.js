      ...
      <Text style={styles.table_content_title}>{translations?.tableContent}</Text>
      {
        questions?.map((question, index) => {
          if (question?.category === 'section') {
            return <Text key={index} style={styles.table_content_section}>{convertToRoman(question?.sectionIndex)} - {question?.title}</Text>
          }
          if (question?.category === 'subsection') {
            return <Text key={index}
              style={styles.table_content_question_subsection}>
              {convertToAlpha(question?.subSectionIndex)}. {question?.title}
            </Text>
          }
          if (question?.category === 'question') {
            return <Text key={index} style={styles.table_content_question_text}>
