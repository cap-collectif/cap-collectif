...
import '~relay/QuestionnaireAdminResultsPieChart_multipleChoiceQuestion.graphql'
...
type Props = {
  ...
  intl: IntlShape
  innerRef: (ref: React.Ref<any> | null | undefined) => void
}
type State = {
...
export class QuestionnaireAdminResultsPieChart extends React.Component<Props, State> {
  ...
  renderCustomizedLabel = ({ cx, cy, midAngle, outerRadius, fill, percent }: Record<string, any>) => {
  ...
  renderCustomizedLabelLine = ({ points, stroke, percent }: Record<string, any>) => {
  ...
  renderTooltip = ({ payload }: { payload: Array<Record<string, any>> }) => {
  ...
  renderLegend = ({ payload }: { payload: Array<Record<string, any>> }) => {
    return (
      <ul>
      ...
      <div className="row">
        <div className="col-xs-12">
          <div
            ...
            style={{
              height: this.getHeight(data.length),
            }}
          >
            <ResponsiveContainer>
