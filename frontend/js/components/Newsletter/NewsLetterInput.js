...
const NewsLetterInput = () => {
  ...
  const onSubmit = (event: Event) => {
    ...
    if (captchaFeature && !captcha) {
      ...
      throw new SubmissionError({ captcha: 'registration.constraints.captcha.invalid' });
    }

    SubscribeNewsletterMutation.commit({ input: { email, captcha } })
      .then(response => {
        setCaptcha('');
        enableCaptcha(false);
        setEmail('');
        ...
        if (response.subscribeNewsletter?.errorCode === 'NEWSLETTER_INVALID_CAPTCHA') {
          toast({
            variant: 'danger',
            content: intl.formatMessage({ id: 'registration.constraints.captcha.invalid' }),
          });
          ...
          toast({
            variant: 'info',
            content: intl.formatMessage({ id: 'newsletter.already_subscribed' }),
          });
        } else {
          toast({
            variant: 'success',
            content: intl.formatMessage({ id: 'homepage.newsletter.success' }),
          });
        }
      })
      .catch(() => {
        ...
        <form onSubmit={onSubmit} id="newsletterSubscriptionForm">
          <Flex mb={2}>
            <Input
              ...
              placeholder={intl.formatMessage({ id: 'homepage.newsletter.enter_email' })}
              aria-label={intl.formatMessage({ id: 'homepage.newsletter.enter_email' })}
          ...
          <div
            className={`form-group ${!captchaFeature ? 'hidden' : ''} ${
              formCaptchaError ? 'has-error' : ''
            }`}>
            <Captcha
              disabled={!captchaEnabled}
              onChange={captchaValue => setCaptcha(captchaValue)}
            />
