...
export const QuestionChoiceAdminModal = ({
  show,
  onClose,
  onSubmit,
  member,
  ...
  const optional = (
    <span className="excerpt"> {intl.formatMessage({ id: 'global.optional' })}</span>
  );
  return (
    <Modal
      show={show}
        ...
        <Modal.Header closeButton>
          <div className="modal-title">
            <Modal.Title id="proposal-form-admin-question-modal-title-lg">
              {intl.formatMessage({ id: !isCreating ? 'global-add-answer' : 'global-edit-answer' })}
            </Modal.Title>
          </div>
        </Modal.Header>
        <Modal.Body>
          ...
          {type === 'button' && (
            <Field
              label={intl.formatMessage({ id: 'global.color' })}
              id={`${member}.color`}
              name={`${member}.color`}
              type="color-picker"
              component={component}
...
export default connect<any, any, _, _, _, _>(mapStateToProps)(QuestionChoiceAdminModal);
