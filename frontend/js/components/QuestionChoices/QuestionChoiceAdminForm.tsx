...
import { FormattedMessage, injectIntl } from 'react-intl' // TODO https://github.com/cap-collectif/platform/issues/7774
...
type QuestionChoices = ReadonlyArray<{
  ...
  readonly image: Record<string, any> | null | undefined
}>
...
type State = {
  editIndex: number
  editMember: string | null | undefined
...
export class QuestionChoiceAdminForm extends React.Component<Props, State> {
  ...
  render() {
    const { dispatch, fields, choices, formName, oldMember, type, intl, importedResponses } = this.props
    const { editIndex, showModal, showAll, editMember, showQuestionChoiceModal, isCreating, choicesWithJump } =
      this.state
      ...
      <div className="form-group" id="questions_choice_panel_personal">
        {showModal && (
          <QuestionnaireAdminModalImportResponses
            ...
            oldMember={oldMember}
            onAdd={() => {
              const responses = choices ? [...importedResponses.data, ...choices] : [...importedResponses.data]
                ...
                <Row>
                  ...
                  <Col xs={4}>
                    <ButtonToolbar className="pull-right">
                      ...
                      {choicesWithJump?.[index]?.destinations && choicesWithJump[index].destinations.length > 0 ? (
                          ...
                          onClick={() => {
                            if (
                              window.confirm(
                                ...
                                intl.formatMessage({
                                  id: 'responses.alert.delete.bodyText',
                                }),
                              )
                            ) {
                              fields.remove(index)
        ...
        <Button
          ...
          onClick={() => {
            dispatch(
              arrayPush(formName, `${oldMember}.choices`, {
