...
const OpinionVersionCreateModal = ({ dispatch, opinion, submitting, show }: Props) => {
  const intl = useIntl()
  const disabled = opinion.step.requirements && !opinion.step.requirements.viewerMeetsTheRequirements
    ...
    <Modal animation={false} show={show} onHide={onClose} bsSize="large" aria-labelledby="contained-modal-title-lg">
      ...
      <Modal.Footer>
        ...
        <Button
          ...
          bsStyle="primary"
        >
          {submitting ? <FormattedMessage id="global.loading" /> : <FormattedMessage id="global.send" />}
        </Button>
      </Modal.Footer>
    </Modal>
  )
...
// @ts-ignore
const container = connect<any, any>(mapStateToProps)(OpinionVersionCreateModal)
