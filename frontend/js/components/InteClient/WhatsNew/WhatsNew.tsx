...
type NewsBlock = {
  ...
  readonly isButtonPlain: boolean
}
...
export const WhatsNew = ({ newsArray }: Props) => {
  ...
  return (
    <Flex direction="column" spacing={10}>
      {newsArray.map(({ title, body, url, cover, hasSeparator, isButtonPlain }: NewsBlock, idx: number) => (
        <Flex
          as={isMobile ? 'a' : 'div'}
        ...
        >
          ...
          <Flex direction="column" flex="1 1 0" m={[0, 5]}>
            ...
            />
            {hasSeparator && (
              <AppBox className="separator-news" width="64px" height="6px" bg="aqua.500" mb={[0, 4]} ml={[4, 0]} />
            )}
            <Text
              mb={[5, 5]}
              m={[5, 0]}
