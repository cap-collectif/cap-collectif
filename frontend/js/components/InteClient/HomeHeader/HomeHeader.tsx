  ...
  <Container style={style}>
    <Content mainLinkColors={mainLink.colors} secondLinkColor={secondLink.textColor}>
      <Tag colors={config.isMobile ? tag.mobileColors : tag.colors}>
        {tag.icon && (
          <Icon name={tag.icon} size={24} color={config.isMobile ? tag.mobileColors.text : tag.colors.text} />
        )}
        <span>{tag.text}</span>
      </Tag>
    ...
    <Illustration img={img}>{!config.isMobile && <Image src={img} alt="" className="illustration" />}</Illustration>
  </Container>
)
