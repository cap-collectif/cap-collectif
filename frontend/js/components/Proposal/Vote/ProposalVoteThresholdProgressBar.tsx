...
export class ProposalVoteThresholdProgressBar extends React.Component<Props> {
  render() {
    ...
    const voteCountLabel = isInterpellationContextFromProposal(proposal) ? 'support.count' : 'vote.count'
    return (
      <div
        className="card__threshold"
        style={{
          fontSize: '85%',
          marginTop: '15px',
        }}
      >
        ...
        <ProgressBar
          ...
          className="mb-0"
          bsStyle="success"
          label={votesCount >= voteThreshold ? <FormattedMessage id="proposal.vote.threshold.reached" /> : ''}
        />
      </div>
    )
  }
