...
import {
  CapUISpotIcon, CapUISpotIconSize, Flex, FormLabel, InputGroup, SpotIcon, Text,
  MultiStepModal, Heading, Button, useMultiStepModal, CapUIIcon
} from '@cap-collectif/ui';
import {type IntlShape, useIntl, FormattedHTMLMessage} from 'react-intl';
...
export const onSmsVoteSubmit = async (
  data: { phone: string, countryCode: string, stepId: string, proposalId: string },
  goToNextStep: () => void,
  ...
  try {
    const response = await SendSmsProposalVoteMutation.commit({ input })
    ...
    if (['INVALID_NUMBER', 'PHONE_INVALID_LENGTH'].includes(errorCode)) {
      setError('phone', {type: 'custom', message: intl.formatMessage({id: 'invalid-format'})})
      return;
    }
    if (errorCode === 'PHONE_SHOULD_BE_MOBILE_NUMBER') {
      setError('phone', {type: 'custom', message: intl.formatMessage({id: 'phone.validation.mobile.format'})})
      return;
    }
    if (errorCode === 'PHONE_ALREADY_USED_BY_ANOTHER_USER') {
      setError('phone', {type: 'custom', message: intl.formatMessage({id: 'phone.validation.number.already.used'})})
      return;
    }
    setIsLoading(false);
    goToNextStep();
    ...
    <>
      <ResetCss>
        <MultiStepModal.Header>
          ...
          <Heading>{intl.formatMessage({ id: 'youre-almost-there' })}</Heading>
        </MultiStepModal.Header>
      </ResetCss>
      <MultiStepModal.Body>
        <Flex as="form" direction="column" spacing={3} align="center" justify="center">
          ...
          <Flex align="center">
            <FormControl
              name="consentSmsCommunication"
              control={control}
              isDisabled={isSubmitting}>
              <FieldInput type="checkbox" name="consentSmsCommunication" control={control}>
                {intl.formatMessage({id: 'i-would-like-to-receive-news-about-platform-projects'}, {platformName})}
              </FieldInput>
            </FormControl>
          </Flex>
        </Flex>
