...
export const ProposalVoteButtonWrapperFragment = ({
  ...
  const hasReachedLimit = (): boolean => {
    ...
    if (step?.viewerVotes?.totalCount !== undefined) {
      return !hasVoted && (votesLimit || false) && (votesLimit || 0) - step.viewerVotes.totalCount <= 0
    }
  ...
  if (step?.voteType === 'SIMPLE') {
    return (
      ...
      >
        <HoverObserver>
          <ProposalVoteButton id={id} proposal={proposal} currentStep={step} disabled={false} hasVoted={hasVoted} />
        </HoverObserver>
      </VoteButtonOverlay>
    )
  }
