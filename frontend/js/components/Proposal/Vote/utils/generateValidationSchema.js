  ...
  return yup.object().shape({
    ...('IdentificationCodeRequirement' in initialValues && {
      IdentificationCodeRequirement: yup
        .string()
        .min(CODE_MINIMAL_LENGTH, intl.formatMessage({ id: 'BAD_CODE' }))
    ...
    ...('PhoneVerifiedRequirement' in initialValues && {
      PhoneVerifiedRequirement: yup.object({
        CountryCode: yup.string().required(intl.formatMessage({ id: 'global.required' })),
        phoneNumber: yup
          .string()
          .ensure()
          .min(10, intl.formatMessage({ id: 'profile.constraints.phone.invalid' }))
          .max(12, intl.formatMessage({ id: 'profile.constraints.phone.invalid' }))
          .matches('^0(6|7)', intl.formatMessage({ id: 'phone.validation.start.with.mobile' }))
          // TODO fix match regex to check if string has whitespace
          .matches('\\d+$', intl.formatMessage({ id: 'invalid-format' })),
      }),
    }),
    ...
    ...('DateOfBirthRequirement' in initialValues && {
      DateOfBirthRequirement: yup.date().required(intl.formatMessage({ id: 'global.required' })),
    }),
