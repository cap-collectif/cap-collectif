...
const getInitialValues = (
  ...
  return isPhoneVerificationOnly
    ...
    ? requirements?.reduce((acc: Record<string, any>, requirement) => {
        ...
        if (requirement.__typename === 'DateOfBirthRequirement') {
          return {
            ...acc,
            [requirement.__typename]: requirement.viewerDateOfBirth ? moment(requirement.viewerDateOfBirth) : null,
          }
        }

        if (requirement.__typename === 'IdentificationCodeRequirement') {
          return { ...acc, IdentificationCodeRequirement: requirement.viewerValue ? requirement.viewerValue : '' }
        }
        ...
        return { ...acc, [requirement.__typename]: requirement.viewerValue }
      }, {})
    : {}
}
