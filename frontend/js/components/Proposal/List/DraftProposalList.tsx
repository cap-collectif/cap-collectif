        ...
        <DraftBox>
          <ul className={classes}>
            {proposals.map((edge, i) => (
              <DraftProposalPreview key={`draft-proposal-${i}`} proposal={edge.node} stepUrl={step.url} />
            ))}
          </ul>
        </DraftBox>
      </Box>
...
export default createFragmentContainer(DraftProposalList, {
  step: graphql`
    fragment DraftProposalList_step on CollectStep @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
      url
