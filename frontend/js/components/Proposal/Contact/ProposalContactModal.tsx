...
export const ProposalContactModal = ({ show, onClose, authorName, addCaptchaField, proposalId }: Props) => {
  const intl = useIntl()
  ...
  const onSubmit = (values: FormValues) => {
    if (!values) return
    ContactProposalAuthorMutation.commit({
      input: { ...values, captcha },
    })
      .then(response => {
        onClose()
        reset()
      ...
      <form onSubmit={e => handleSubmit((data: FormValues) => onSubmit(data))(e)} id={formName}>
        ...
        <Modal.Body>
          ...
          <FormControl name="replyEmail" control={control} isRequired>
            ...
            />
            <FieldInput id="ProposalFormContactModal-replyEmail" name="replyEmail" control={control} type="email" />
          </FormControl>
          ...
          <FormControl name="message" control={control} isRequired>
            ...
            />
            <FieldInput id="ProposalFormContactModal-message" name="message" control={control} type="textarea" />
          </FormControl>

          {addCaptchaField && <Captcha onChange={captchaKey => setCaptcha(captchaKey)} />}
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
