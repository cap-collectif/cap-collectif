...
import { submit, SubmissionError, isPristine, isDirty, isInvalid, isSubmitting, hasSubmitFailed } from 'redux-form'
...
`
export type BackOfficeFormValues = FormValues & {
  author: {
    value: Uuid
    label: string
  }
...
const onSubmit = (
  ...
  return CreateProposalFromBackOfficeMutation.commit({
    ...
    .then(response => {
      if (!response.createProposalFromBackOffice || !response.createProposalFromBackOffice.proposal) {
        onSubmitFailed()
        toast({
          variant: 'danger',
          position: 'top',
      ...
      const backOfficeRedirect =
        proposalForm.step?.id && proposalForm.step?.slug && proposalForm.step?.project?._id
          ...
          : document.referrer
      const redirectUrl = document.referrer.indexOf(backOfficeRedirect) === 0 ? document.referrer : backOfficeRedirect
      toast({
        variant: 'success',
        position: 'top',
        content: intl.formatHTMLMessage({
...
export const ProposalAdminContentCreateForm = ({
  ...
  const confirmModal = (
    disclosure: React.ReactElement<React.ComponentProps<any>, any>,
    ...
    return (
      <ConfirmModal
        ...
        body={intl.formatMessage({
          id: 'information-should-not-be-saved',
        })}
  ...
  return (
    <Flex m={6} direction="column">
      ...
      >
        ...
        >
          {intl.formatMessage({
            id: isLoading ? 'global.creation' : 'global.create',
          })}
        </Button>
        {dirty ? (
          confirmModal(
          ...
          <Button
            ...
            variantColor="primary"
            onClick={() => {
              if (proposalForm.step?.id && proposalForm.step?.slug && proposalForm.step?.project?._id) {
