...
export const ProposalAdminOfficialAnswer = ({ proposal, viewer }: Props) => {
  ...
  return (
    <div className="mt-20">
        ...
        <>
          <Container>
            <div>
              ...
              <Label color={isFutur ? styleGuideColors.orange150 : styleGuideColors.green150} fontSize={12}>
                <FormattedMessage
                  id={isFutur ? 'global.plannedDate.plannedTime.feminine' : 'global.publishDate.publishTime.feminine'}
                  values={{
                    date: (
                      <FormattedDate
                    ...
                    time: (
                      <FormattedDate value={moment(officialResponse?.publishedAt)} hour="numeric" minute="numeric" />
                    ),
                  }}
                />
              </Label>
      ...
      {mode === 'EDIT' && (
        <ProposalAdminOfficialAnswerForm proposal={proposal} viewer={viewer} onValidate={() => setMode('VIEW')} />
      )}
      {mode === 'NONE' && (
        <InfoContainer>
