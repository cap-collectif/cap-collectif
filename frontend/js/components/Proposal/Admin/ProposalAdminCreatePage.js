...
export type Props = {| stepId: number |};

const createComponent = ({error, props}: { error: ?Error, props: any }) => {
  if(error) {
  ...
  return (
    <div className="admin_proposal_form">
      <QueryRenderer
        ...
        query={ graphql`
              query ProposalAdminCreatePageQuery(
                $stepId: ID!
              ) {
                step: node(id: $stepId) {
                  id
...
export default connect<any, any, _, _, _, _>()(ProposalAdminCreatePage);
