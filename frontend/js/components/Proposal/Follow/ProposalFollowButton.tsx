...
const FRAGMENT = graphql`
  fragment ProposalFollowButton_proposal on Proposal @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
    id
...
const ProposalFollowButton = ({ proposal: proposalFragment }: Props) => {
  ...
  if (!proposal)
    return (
      <Button disabled className="btn btn-default proposal__button__follow" id="proposal-follow-btn-placeholder">
  ...
  if (proposal.viewerFollowingConfiguration !== null && typeof proposal.viewerFollowingConfiguration !== 'undefined') {
    return (
      <LoginOverlay>
        <Menu
          placement="bottom"
