...
const ProposalAnalysisCommentCreateForm = ({
  ...
  const [buttonBackgroundColor, buttonTextColor] = useSelector(state => {
    return [
      state.default.parameters['color.btn.primary.bg'],
      state.default.parameters['color.btn.primary.text'],
    ];
  });
  ...
  const connectionId = formatConnectionPath(
    ['client', proposalAnalysis.id],
    'ProposalAnalysis_comments',
  );
      ...
      <Flex>
        ...
        <Flex id="create-analysis-comment-form" as="form" direction="column" width="100%" action="">
          <TextArea
            ...
            placeholder={intl.formatMessage({ id: 'write-a-comment' })}
            required
          />
          {hasTyped && (
            <Box width="100%" mt={2}>
              <Button
                ...
                {intl.formatMessage({ id: 'comment.submit' })}
              </Button>
            </Box>
          )}
        </Flex>
