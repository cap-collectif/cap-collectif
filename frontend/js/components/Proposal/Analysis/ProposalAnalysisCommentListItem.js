...
import {
  Box,
  ButtonQuickAction as ButtonQuickActionDS,
  CapUIIcon,
  Flex,
  Text,
...
const ProposalAnalysisCommentListItem = ({
  comment: commentRef,
  viewer: viewerRef,
  connectionId,
}: Props) => {
  const intl = useIntl();
  const comment = useFragment<ProposalAnalysisCommentListItem_comment$key>(
    COMMENT_FRAGMENT,
    commentRef,
  );
  ...
  const buttonBackgroundColor = useSelector(
    state => state.default.parameters['color.btn.primary.bg'],
  );
