...
type Props = {|
  +proposalAnalysis: ProposalAnalysisComments_proposalAnalysis$key,
  +viewer: ProposalAnalysisComments_viewer$key,
|};
...
const ProposalAnalysisComments = ({
  proposalAnalysis: proposalAnalysisRef,
  viewer: viewerRef,
}: Props) => {
    ...
    <Flex direction="column" backgroundColor="#F4F4F4" p={4}>
      <Box mb={4}>
        <Text color="neutral-gray.900" fontSize={3} fontWeight={600}>
          {intl.formatMessage({ id: 'admin.label.comment' })}
        </Text>
        <Text color="neutral-gray.700" fontSize={2}>
          {intl.formatMessage({ id: 'space-for-the-analyst-supervisor-decision-maker' })}
        </Text>
      </Box>
      {canComment && (
        <ProposalAnalysisCommentCreateForm viewer={viewer} proposalAnalysis={proposalAnalysis} />
      )}
