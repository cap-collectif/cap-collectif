...
import type { Step } from '../../Detail/ProposalDetailAdvancement'
...
`
const AdvancementStepPlaceholderContainer = styled.div<{
  isLast?: boolean
}>`
  display: flex;
...
export const ProposalPageAdvancement = ({ proposal }: Props) => {
  ...
  for (const step of steps) {
    step.isSelected =
      step.__typename === 'CollectStep' || proposal?.selections.map(selection => selection.step.id).includes(step.id)
  }
