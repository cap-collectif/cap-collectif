...
import { Box, Text, Button, ButtonGroup, Heading, toast, Modal, CapUIModalSize } from '@cap-collectif/ui'
...
`
type FragmentProps = {
  readonly proposal: ModalProposalSocialNetworks_proposal$key
  readonly proposalType: ModalProposalSocialNetworks_proposal
}
type FormValues = {
...
const onSubmit = (values: FormValues, _dispatch, { proposalId, proposalType, isAuthenticated }: Props) => {
  const data = {
  ...
  return UpdateProposalSocialNetworksMutation.commit({
    ...
    .then(response => {
      if (!response.updateProposalSocialNetworks || !response.updateProposalSocialNetworks.proposal) {
...
const ModalProposalSocialNetworks = ({
  ...
  return (
    ...
    >
      ...
      >
        <AlertForm valid={valid} invalid={invalid} submitFailed={submitFailed} submitting={submitting} />
        <ButtonGroup>
          <Button
            variant="secondary"
            variantSize="big"
