...
type Props = {|
  ...
  +onClose?: () => void,
  +post: ProposalNewsEditModal_post,
  +intl: IntlShape,
|};
...
export const ProposalNewsEditModal = ({
  ...
  const onUpdate = (values: FormValues) => {
    ...
    return UpdateProposalNewsMutation.commit({ input: data })
      .then(() => {
        toast({
          variant: 'success',
          content: intl.formatHTMLMessage({ id: 'your-form-has-been-updated' }),
        });
        displayModal(false);
  ...
  const onDelete = postId => {
    return DeleteProposalNewsMutation.commit({ input: { postId } })
      .then((response: DeleteProposalNewsMutationResponse) => {
        toast({
          variant: 'success',
          content: intl.formatHTMLMessage({ id: 'your-form-has-been-deleted' }),
        });
