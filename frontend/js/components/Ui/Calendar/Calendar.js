...
type Props = {|
  ...
  inputs: Array<{| title: string, date: string |}>,
  activeNumber?: number,
  withBorder?: boolean,
|};

const InputContent: StyledComponent<
  { color: string, last: boolean, branchColor: string },
  {},
  HTMLDivElement,
> = styled.div`
  position: relative;
...
const Circle: StyledComponent<{}, {}, HTMLDivElement> = styled.div`
  margin-right: 15px;
`;

const CircleData: StyledComponent<
  { color: string, active: boolean, backgroundColor: string, withBorder: boolean },
  {},
  HTMLDivElement,
> = styled.div`
  height: 40px;
...
const Data: StyledComponent<{}, {}, HTMLDivElement> = styled.div`
  margin-right: 15px;
`;

const Title: StyledComponent<{ color: string }, {}, HTMLDivElement> = styled.div`
  color: ${props => props.color};
  font-weight: bold;
`;
        ...
        inputs.map((input, idx) => (
          <div key={idx} style={{ margin: '15px 0' }}>
            <InputContent
              ...
              <Data>
                <Title color={idx === activeNumber - 1 ? active : defaultColor}>
                  {input.title}
                </Title>
                <div>{input.date}</div>
              </Data>
