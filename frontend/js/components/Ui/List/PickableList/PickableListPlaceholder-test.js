...
describe('<PickableListPlaceholder />', () => {
  ...
  it('should render correctly with a custom header content', () => {
    const component = shallow(
      <PickableListPlaceholder headerContent={<p>Bla bla bla d'la pookie</p>} />,
    ).render();
  ...
  it('should render correctly with a custom rows count and a custom header', () => {
    const component = shallow(
      <PickableListPlaceholder
        rowsCount={2}
        headerContent={<p>Rem Chan, daiskii uwuuu :3 oniichaaaan</p>}
      />,
    ).render();
    expect(component.find('.pickableList-header').text()).toBe(
      'Rem Chan, daiskii uwuuu :3 oniichaaaan',
    );
