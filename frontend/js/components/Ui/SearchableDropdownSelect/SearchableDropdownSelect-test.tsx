...
describe('<SearchableDropdownSelect />', () => {
  it('should render correctly', () => {
    const wrapper = shallow(
      ...
      >
        {results => results.map(anime => <DropdownSelect.Choice value={anime.id}>{anime.title}</DropdownSelect.Choice>)}
      </SearchableDropdownSelect>,
    )
  ...
  it('should correctly set the values based on defaultOptions for the DropdownSelect.Choice', () => {
    ...
    expect(wrapper.find(DropdownSelectChoice).at(0).prop('value')).toBe('default1')
    expect(wrapper.find(DropdownSelectChoice).at(1).prop('value')).toBe('default2')
  })
  it('should correctly search and retrieve items', async () => {
    ...
    expect(wrapper.find(DropdownSelectChoice).exists()).toBe(true)
    expect(wrapper.find(DropdownSelectChoice).at(0).prop('value')).toBe('anime1')
    // Trigger a "My third anime" search
    wrapper.find('input').simulate('change', {
      target: {
        value: 'My third anime',
      },
    })
    expect(wrapper.find(Loader).exists()).toBe(true)
    await wait(1)
    ...
    expect(wrapper.find(Loader).exists()).toBe(false)
    expect(wrapper.find(DropdownSelectChoice).exists()).toBe(true)
    expect(wrapper.find(DropdownSelectChoice).at(0).prop('value')).toBe('anime3')
    // Trigger a "My" search
    wrapper.find('input').simulate('change', {
      target: {
        value: 'My',
    ...
    expect(wrapper.find(DropdownSelectChoice).exists()).toBe(true)
    expect(wrapper.find(DropdownSelectChoice).at(0).prop('value')).toBe('anime1')
    expect(wrapper.find(DropdownSelectChoice).at(1).prop('value')).toBe('anime2')
    expect(wrapper.find(DropdownSelectChoice).at(2).prop('value')).toBe('anime3')
  })
})
