...
export type Props = AppBoxProps & {
  ...
  circle?: boolean
  label?: string
  format: 'image/*' | 'audio/*' | 'video/*' | Array<string>
...
const Uploader = ({
  ...
  const intl = useIntl()
  const [thumb, setThumb] = React.useState<string | null | undefined>(null)
  const [drag, setDrag] = React.useState<boolean>(false)
  const [warning, setWarning] = React.useState<string | null | undefined>(null)
  ...
  const getContent = () => {
    switch (size) {
      ...
      default:
        return (
          <Content>
            <Icon name={drag ? 'ADD_DRAG' : 'ADD'} size="lg" color={drag ? 'blue.500' : 'gray.500'} />
          </Content>
        )
    }
  }
  ...
  return (
    <UploaderContainer size={size}>
      {!!label && (
        <Flex direction="column" mb={2}>
          ...
          <Flex direction="row" wrap="wrap">
            <Text display="flex" flexDirection="row" marginRight={1} fontSize={1} lineHeight="sm" color="gray.500">
              {intl.formatMessage({
                id: 'uploader.banner.format',
              })}
      ...
      <Errors fileRejections={fileRejections} multiple={multiple} maxSize={maxSize} acceptedFiles={acceptedFiles} />
      {warning && (
        <WarningList>
