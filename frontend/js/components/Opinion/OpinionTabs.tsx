...
class OpinionTabs extends React.Component<Props> {
  ...
  getArgumentsTrad = () =>
    this.getCommentSystem() === COMMENT_SYSTEM_BOTH ? 'global.arguments' : 'global.simple_arguments'
  ...
  isCommentable = () =>
    this.getCommentSystem() === COMMENT_SYSTEM_SIMPLE || this.getCommentSystem() === COMMENT_SYSTEM_BOTH

  isVersionable = () => {
    const { opinion } = this.props
    return (opinion.__typename === 'Opinion' && opinion.section && opinion.section.versionable) || false
  }
  ...
  render() {
    ...
    if (this.isSourceable() + this.isCommentable() + this.isVersionable() + this.isFollowable() > 1) {
      // at least two tabs
      const marginTop = {
        marginTop: '20px',
      }
              ...
              {this.isCommentable() && (
                <NavItem className="opinion-tabs" eventKey="arguments">
                  <FormattedMessage
                    ...
                    values={{
                      num: opinion.allArguments ? opinion.allArguments.totalCount : 0,
                    }}
                  />
                </NavItem>
              )}
              {this.isVersionable() && (
                <NavItem eventKey="versions" className="opinion-tabs">
                  <FormattedMessage
                    id="global.versions"
                    values={{
                      num: opinion.allVersions ? opinion.allVersions.totalCount : 0,
                    }}
                  />
                </NavItem>
              )}
              {this.isSourceable() && (
                <NavItem className="opinion-tabs" eventKey="sources">
                  <FormattedMessage
                    id="global.sources"
                    values={{
                      num: opinion.allSources ? opinion.allSources.totalCount : 0,
                    }}
                  />
                </NavItem>
              )}
              {this.isFollowable() && (
                <NavItem className="opinion-tabs" eventKey="followers">
                  <FormattedMessage
                    ...
                    values={{
                      num: opinion.allFollowers ? opinion.allFollowers.totalCount : 0,
                    }}
                  />
                </NavItem>
              )}
