...
export class OpinionSourceBox extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <div>
        {sourceable.viewerSourcesUnpublished && sourceable.viewerSourcesUnpublished.totalCount > 0 ? (
              ...
              <Panel.Title>
                <strong>
                  <FormattedMessage
                    id="global.sources"
                    values={{
                      num: sourceable.viewerSourcesUnpublished.totalCount,
                    }}
                  />
                </strong>{' '}
            ...
            <ListGroup>
              {sourceable.viewerSourcesUnpublished.edges &&
                sourceable.viewerSourcesUnpublished.edges
                  ...
                  .map(edge => edge.node)
                  .filter(Boolean)
                  .map(source => <OpinionSource key={source.id} source={source} sourceable={sourceable} />)}
            </ListGroup>
          </Panel>
        ) : null}
        <Panel>
          ...
          <QueryRenderer
            ...
            variables={
              ...
              } as OpinionSourceBoxQueryVariables
            }
            render={({
              error,
              props,
            }: ReactRelayReadyState & {
              props?: OpinionSourceBoxQueryResponse | null | undefined
            }) => {
              if (error) {
                return graphqlError
              }
...
export default createFragmentContainer(OpinionSourceBox, {
  sourceable: graphql`
    fragment OpinionSourceBox_sourceable on Sourceable @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
      id
