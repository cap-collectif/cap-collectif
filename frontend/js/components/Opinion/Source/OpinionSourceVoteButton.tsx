...
export class OpinionSourceVoteButton extends React.Component<Props, State> {
  ...
  onClick = () => {
    ...
    if (source.viewerHasVote) {
      return RemoveSourceVoteMutation.commit({
        input: {
          sourceId: source.id,
        },
      })
    ...
    return AddSourceVoteMutation.commit({
      input: {
        sourceId: source.id,
      },
    })
...
export default createFragmentContainer(OpinionSourceVoteButton, {
  source: graphql`
    fragment OpinionSourceVoteButton_source on Source @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
      id
      contribuable
      step {
        requirements {
