...
export class OpinionButtons extends React.Component<Props> {
  ...
  renderEditButton = () => {
    ...
    if (opinion.contribuable && this.isTheUserTheAuthor()) {
      ...
      return <OpinionEditButton className="opinion__action--edit btn--outline btn-dark-gray" opinion={opinion} />
    }
  }
    ...
    return (
      <Flex direction="row" justify="space-between">
        <Flex direction="row" spacing={1}>
          {opinion.project && opinion.project.opinionCanBeFollowed && <OpinionFollowButton opinion={opinion} />}
          <OpinionReportButton opinion={opinion} />
          {opinion.title && opinion.url && (
            <ShareButtonDropdown id="opinion-share-button" className="ml-5" title={opinion.title} url={opinion.url} />
          )}
        </Flex>
        <Flex direction="row" spacing={1}>
...
// @ts-ignore
const container = connect<any, any>(mapStateToProps)(OpinionButtons)
export default createFragmentContainer(container, {
  opinion: graphql`
    fragment OpinionButtons_opinion on OpinionOrVersion @argumentDefinitions(isAuthenticated: { type: "Boolean!" }) {
