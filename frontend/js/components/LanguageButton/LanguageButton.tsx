...
export const LanguageButton = ({ onChange, languages, defaultLanguage }: Props) => {
  const languagesFormatted = languages.map(language => ({ ...language, code: formatLanguageCode(language.code) }))
  const [currentLanguage, updateLanguage] = useState(languagesFormatted.find(l => l.code === defaultLanguage))
