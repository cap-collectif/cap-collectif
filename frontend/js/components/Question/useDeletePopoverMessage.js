...
export const useDeletePopoverMessage = (question: Question) => {
  ...
  const [destinationJumpsKey, setDestinationJumpsKey] = useState(
    'admin.jump.delete.confirmation.body',
  );
  ...
  useEffect(() => {
    ...
    setDestinationJumpsText([
      ...new Set(question?.destinationJumps?.map(jump => `"${jump.origin.title}"`)),
    ]);
