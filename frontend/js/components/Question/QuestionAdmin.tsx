...
const getIdFromQuestionType = (type: QuestionTypeValue, level: number | null | undefined): string => {
...
export const handleConfirmDelete = (
  ...
  if (question.destinationJumps.length > 0) {
    question.destinationJumps.forEach(destinationJump => {
      ...
      const remainingJumps = questions[originIndex].jumps.filter(j => j.destination.id !== question.id)
...
export const QuestionAdmin = ({
  ...
  const { jumpsText, jumpsTextParams, destinationJumpsText, destinationJumpsTextParams } =
    useDeletePopoverMessage(question)
  const [popoverHeaderKey, setPopoverHeaderKey] = useState('admin.question.delete.confirmation.jump.header')
  useEffect(() => {
    ...
    <Row>
      <Col xs={8} className="d-flex align-items-center">
        ...
        <ItemQuestionWrapper ellipsis>
          ...
          <br />
          <span className="excerpt">{questionTypeId !== '' && <FormattedMessage id={questionTypeId} />}</span>
        </ItemQuestionWrapper>
      </Col>
      <Col xs={4}>
        <ActionIconWrapper>
                    ...
                    <React.Fragment>
                      ...
                      <Popover.Body>
                        <Text>
                          ...
                          {jumpsText.length > 0 && destinationJumpsText.length === 0 && jumpsTextParams && (
                            // @ts-ignore
                            <FormattedMessage {...jumpsTextParams} />
                          )}
                          {destinationJumpsText.length > 0 &&
                            jumpsText.length === 0 &&
                            destinationJumpsTextParams && ( // @ts-ignore
                              <FormattedMessage {...destinationJumpsTextParams} />
                            )}
                        </Text>
                      </Popover.Body>
