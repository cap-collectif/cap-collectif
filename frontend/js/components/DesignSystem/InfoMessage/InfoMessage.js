...
export type InfoMessageProps = {|
  ...
  children: React.ChildrenArray<
    React.Element<typeof InfoMessageTitle> | React.Element<typeof InfoMessageContent>,
  >,
  variant: 'info' | 'infoGray' | 'danger' | 'success' | 'warning',
|};

const InfoMessageInner: StyledComponent<{}, {}, any> = styled(Flex)(
  variantStyle({
    variants: {
      info: {
...
const InfoMessage = ({ children, variant, ...props }: InfoMessageProps) => {
  ...
  return (
    <InfoMessageInner
      ...
      variant={variant}
      {...props}>
      {titleChild && React.cloneElement((titleChild: React.Element<any>), { variant })}
      {contentChild}
    </InfoMessageInner>
  );
};
