...
export type TagVariant = 'blue' | 'aqua' | 'red' | 'green' | 'orange' | 'yellow' | 'gray' | 'neutral-gray'
...
const Tag = React.forwardRef<HTMLSpanElement, Props>(
  ({ children, icon, onRemove, avatar, variantType = 'tag', interactive = true, ...props }: Props, ref) => {
    ...
    return (
      ...
      >
        {avatar && (avatar.src || avatar.name) && (
          <Avatar {...avatar.props} size="xs" mr={1} src={avatar.src} name={avatar.name} alt={avatar.alt} />
        )}
