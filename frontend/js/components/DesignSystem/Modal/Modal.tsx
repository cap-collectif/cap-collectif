...
export type ModalProps = AppBoxProps & {
  ...
  readonly children?: RenderProps | React.ReactNode | JSX.Element | JSX.Element[]
...
const ModalInner = styled(motion(AppBox)).attrs(({ fullSizeOnMobile, fullPageScrollable, ...rest }) => ({
  display: 'flex',
  bg: 'white',
  flexDirection: 'column',
...
const Modal = ({
  ...
  const $container = useRef<HTMLElement | null | undefined>()
  const $scrollBox = useRef<HTMLElement | null | undefined>()
