...
type Props = FlexProps & {
  children: React.ChildrenArray<React.ReactElement<typeof AccordionItem> | null>
...
const getAccordion = (id, accordion, allowMultiple) =>
  Object.entries(accordion).reduce(
    (acc, [key, value]) => {
      if (!allowMultiple) {
        return { ...acc, [key]: id !== key ? false : !value }
      }

      return { ...acc, [key]: id === key ? !value : value }
    },
    { ...accordion },
  )

const Accordion = ({ children, allowMultiple, defaultAccordion, ...props }: Props) => {
  const [accordion, updateAccordion] = React.useState(getDefaultAccordion(children, defaultAccordion))
