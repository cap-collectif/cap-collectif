...
export const updateQueryUrl = (url: URL, key: string, options: UpdateQueryUrlOptions) => {
  ...
  if (options.delete) {
    ...
    if (typeof options.value === 'string') {
      ...
      url.searchParams.delete(key);
      ((options.value: any): string[]).forEach(id => url.searchParams.append(key, id));
    }
  }
...
export const clearQueryUrl = (url: URL, options?: { replaceState: boolean }) => {
  if (window.location.href) {
    // update the url href, e.g when navigating with react-router, to keep the value up to date
    url.href = window.location.href;
  }
...
export const getFieldsFromUrl = <T: {}>(url: URL, options?: FiltersFromUrlOptions<T>): T => {
  if (window.location.href) {
    // update the url href, e.g when navigating with react-router, to keep the value up to date
    url.href = window.location.href;
  }
