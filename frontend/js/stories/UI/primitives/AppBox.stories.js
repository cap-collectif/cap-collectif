...
const ResponsiveTemplate = (args: any) => (
  <AppBox display="flex" gridGap={2} {...args}>
    ...
    <AppBox p={2} bg={{ _: 'red.300', md: 'yellow.300', lg: 'green.300' }}>
      With named response parameters. I will be red.300 on mobile , yellow.300 on md breakpoint, and
      green.300 on lg breakpoint
    </AppBox>
    <AppBox p={2} bg={['red.300', 'yellow.300', 'green.300']}>
      With array parameters. It goes from the lower breakpoint and goes to the next breakpoint. I
      will be red.300 on lowest breakpoint, then yellow.300 on the tablet breakpoint and finally
      green.300 in the desktop breakpoint
    </AppBox>
    <AppBox p={2} bg={['red.300', null, 'green.300']}>
      You can also skip some breakpoint with the array parameter by providing a null value. Here, it
      will skip the tablet breakpoint (and thus keeping the red.300 value until desktop)
    </AppBox>
  </AppBox>
);
...
const CustomCSSTemplate = (args: any) => (
  <AppBox display="flex" gridGap={2} {...args}>
    ...
    <AppBox
      p={2}
      bg="yellow.400"
      css={({ theme }) => ({ 'p:last-of-type': { color: theme.colors.red[400] } })}>
      <p>Using css function</p>
