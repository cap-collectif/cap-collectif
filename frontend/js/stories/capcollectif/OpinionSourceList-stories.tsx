  ...
  <Media>
    ...
    <Media.Body className="opinion__body">
      ...
      <div className="small">
        ...
        {item.user && item.user.isViewer && (
          <React.Fragment>
            <button type="button" className="source__btn--delete btn btn-xs btn-danger btn--outline" onClick={() => {}}>
  ...
  <div>
    <Panel>
      <Panel.Heading>
        <Row>
          <Col xs={12} sm={6} md={6}>
            <Button id="source-form__add" disabled={!section.addSourceEnable} bsStyle="primary" onClick={() => {}}>
          ...
          <ListGroup id="sources-list">
            {opinionSources.map((item, index) => (
              <ListGroupItem
                key={index}
                id={`source-${index}`}
                className={`list-group-item__opinion opinion ${item.user && item.user.vip ? ' bg-vip' : ''}`}
                style={{
                  backgroundColor: item.user && item.user.vip ? '#F7F7F7' : undefined,
                }}
              >
                <OpinionSourceItem item={item} isProfile={isProfile} typeLabel={section.typeLabel} />
              </ListGroupItem>
            ))}
...
storiesOf('Cap Collectif/OpinionSourceList', module)
  .add('default case', () => {
    ...
    return <OpinionSourceList section={section} opinionSources={opinionSourcesMock} isProfile={false} />
  })
  .add('empty', () => {
    const section = {
      addSourceEnable: boolean('Add source enabled', true, 'Section'),
  ...
  .add('with single opinion source', () => {
    ...
    return <OpinionSourceList section={section} opinionSources={[opinionSourcesMock[0]]} isProfile={false} />
  })
  .add('in profile', () => {
    const section = {
      addSourceEnable: boolean('Add source enabled', true, 'Section'),
