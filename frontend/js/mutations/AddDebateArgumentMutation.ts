...
const mutation = graphql`
  mutation AddDebateArgumentMutation($input: CreateDebateArgumentInput!, $connections: [ID!]!, $edgeTypeName: String!) {
  ...
  commitMutation(environment, {
    ...
    updater: (store: RecordSourceSelectorProxy) => {
      ...
      if (debateAlternateArgumentsConnection) {
        ...
        const nodeForAndAgainst = store.create(`client:newNode:${new Date().toISOString()}`, 'DebateArgumentAlternate')
        ...
        ConnectionHandler.insertEdgeBefore(debateAlternateArgumentsConnection, edgeAlternateArgument)
      }
    },
  })
