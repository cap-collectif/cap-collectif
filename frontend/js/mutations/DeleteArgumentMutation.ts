...
const commit = (
  ...
  commitMutation(environment, {
    ...
    updater: (store: RecordSourceSelectorProxy) => {
      ...
      if (published) {
        const connection = ConnectionHandler.getConnection(argumentableProxy, 'ArgumentList_allArguments', {
          type,
        })
        ...
        const allArgumentsProxy = argumentableProxy.getLinkedRecord('arguments', {
          first: 0,
        })
      ...
      if (!published) {
        ...
        if (!connection) {
          throw new Error('Expected "UnpublishedArgumentList_viewerArgumentsUnpublished" to be in the store')
        }
