...
type Variables = ReportDebateArgumentMutationVariables & {
  debateId: string
    ...
    updater: (store: RecordSourceSelectorProxy) => {
      ...
      if (variables.isMobile) {
        ...
        const nodeWithReportedArgument = edges
          ...
          .find(
            argument =>
              argument && argument.getLinkedRecord(variables.forOrAgainst.toLowerCase())?.getValue('id') === argumentId,
          )
        if (!nodeWithReportedArgument) throw new Error("Node with argument reported doesn't exist ")
        const reportedArgument = nodeWithReportedArgument.getLinkedRecord(variables.forOrAgainst.toLowerCase())
        ...
        const debateArguments = ConnectionHandler.getConnection(debate, 'DebateStepPageArgumentsPagination_arguments', {
