...
const commit = (variables: Variables): Promise<DeleteEmailingCampaignMutationResponse> =>
  commitMutation(environment, {
    ...
    updater: store => {
      ...
      const deletedEmailingCampaignsIds = store
        .getRootField('deleteEmailingCampaigns')
        .getValue('deletedIds');

      const archivedEmailingCampaignsIds = store
        .getRootField('deleteEmailingCampaigns')
        .getValue('archivedIds');

      [...deletedEmailingCampaignsIds, ...archivedEmailingCampaignsIds].forEach(
        campaignDeletedOrArchived => {
      ...
      const countCampaignsDeleted =
        deletedEmailingCampaignsIds.length + archivedEmailingCampaignsIds.length;

      // Update count
      const countCampaigns = ((campaigns.getValue('totalCount'): any): number);
      campaigns.setValue(countCampaigns - countCampaignsDeleted, 'totalCount');

      const campaignAll = root.getLinkedRecord('emailingCampaigns', { status: null });
      campaignAll.setValue(countCampaigns - countCampaignsDeleted, 'totalCount');

      const campaignSent = root.getLinkedRecord('emailingCampaigns', { status: 'SENT' });
      ...
      const campaignPlanned = root.getLinkedRecord('emailingCampaigns', { status: 'PLANNED' });
      const countCampaignPlanned = campaignPlanned.getValue('totalCount');
      campaignPlanned.setValue(
        countCampaignPlanned - allCountCampaignsDeleted.PLANNED,
        'totalCount',
      );

      const campaignDraft = root.getLinkedRecord('emailingCampaigns', { status: 'DRAFT' });
