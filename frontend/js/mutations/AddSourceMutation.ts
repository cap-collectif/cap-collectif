...
import type { AddSourceMutationVariables, AddSourceMutationResponse } from '~relay/AddSourceMutation.graphql'
...
const commit = (
  ...
  commitMutation(environment, {
    ...
    updater: (store: RecordSourceSelectorProxy) => {
      ...
      if (viewerIsConfirmed) {
        const allSourcesProxy = sourceableProxy.getLinkedRecord('sources', {
          first: 0,
        })
        ...
        const connection = ConnectionHandler.getConnection(sourceableProxy, 'OpinionSourceBox_viewerSourcesUnpublished')
