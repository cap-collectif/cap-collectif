...
const updater = (variables: Variables, store: RecordSourceSelectorProxy) => {
  ...
  const countProposals = ((proposals.getValue('totalCount'): any): number);
  ...
  if (proposalsCurrentState) {
    proposalsCurrentState.setValue(((proposals.getValue('totalCount'): any): number), 'totalCount');
  }

  // Update count trash
  const proposalsTrashed = project.getLinkedRecord('proposals', { state: 'TRASHED' });

  if (proposalsTrashed) {
    const countProposalsTrashed = ((proposalsTrashed.getValue('totalCount'): any): number);
...
const updaterAnalysis = (variables: Variables, store: RecordSourceSelectorProxy) => {
  ...
  const proposals = ConnectionHandler.getConnection(
    analysisStep,
    'ProjectAdminAnalysis_proposals',
    {
      orderBy: {
        field: 'PUBLISHED_AT',
        direction: parameters.sort === 'newest' ? 'DESC' : 'ASC',
