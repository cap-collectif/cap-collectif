...
`
type Variables = AddProposalsFromCsvMutationVariables & {
  projectId?: string
...
const commit = (variables: Variables): Promise<AddProposalsFromCsvMutationResponse> =>
  commitMutation(environment, {
    ...
    updater: (store: RecordSourceSelectorProxy) => {
      ...
      const proposalConnection = ConnectionHandler.getConnection(project, 'ProjectAdminProposals_proposals', {
        orderBy: {
          field: 'PUBLISHED_AT',
      ...
      importedProposalsArray.map(proposal => {
        if (!proposal) return
        // @ts-expect-error return type is mixed
        const newProposalId: string | null | undefined = proposal.getValue('id')
