...
type Variables = DeleteAnonymousReplyMutationVariables & {
...
const commit = (variables: Variables): Promise<DeleteAnonymousReplyMutationResponse> =>
  commitMutation(environment, {
    ...
    updater: (store: RecordSourceSelectorProxy) => {
      ...
      const args = {
        ids: variables.anonymousRepliesIds,
      }
      const anonymousReplies = rootFields.getLinkedRecords('nodes', args)
      const updatedAnonymousReplies = anonymousReplies
        ? anonymousReplies.filter(replyRecordProxy => !!replyRecordProxy && replyRecordProxy?.getDataID() !== replyId)
        : []
